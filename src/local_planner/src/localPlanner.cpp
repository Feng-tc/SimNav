...
string pathFolder;
string odomTopic = "/Odometry_gazebo";
...
double goalY = 0;
double goalZ = 0;
bool preventLowerFloorPlanning = true;
double maxAllowedDrop = 0.05;
...
float startPathMinRelZ[groupNum] = {0};
...
void readStartPaths()
{
  ...
  for (int i = 0; i < pointNum; i++) {
    ...
    if (groupID >= 0 && groupID < groupNum) {
      ...
      if (point.z < startPathMinRelZ[groupID]) {
        startPathMinRelZ[groupID] = point.z;
      }
    }
  }
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("preventLowerFloorPlanning", preventLowerFloorPlanning);
  nhPrivate.getParam("maxAllowedDrop", maxAllowedDrop);

  ros::Subscriber subOdometry = nh.subscribe<nav_msgs::Odometry>
                                (odomTopic, 5, odometryHandler);

  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>
                                  ("/livox/Pointcloud2", 5, laserCloudHandler);
  ...
  readStartPaths();
  for (int i = 0; i < groupNum; i++) {
    if (startPathMinRelZ[i] > 999.0) {
      startPathMinRelZ[i] = 0;
    }
  }
  #if PLOTPATHSET == 1
  readPaths();
  ...
  while (status) {
    ...
    if (newLaserCloud || newTerrainCloud) {
      ...
      if (autonomyMode) {
        ...
        if (preventLowerFloorPlanning && goalZ < vehicleZ - maxAllowedDrop) {
          relativeGoalDis = 0;
        }
      }
      ...
      while (pathScale >= minPathScale && pathRange >= minPathRange) {
        ...
        for (int i = 0; i < 36 * groupNum; i++) {
          ...
          int groupID = i % groupNum;
          if (rotDeg > 180.0) rotDeg -= 360.0;
          if (maxScore < clearPathPerGroupScore[i] &&
              (!preventLowerFloorPlanning || startPathMinRelZ[groupID] >= -maxAllowedDrop) &&
