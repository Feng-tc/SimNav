...
// ==================== Constructor ====================
RoomSegmentationNode::RoomSegmentationNode(ros::NodeHandle& nh, ros::NodeHandle& pnh)
    : nh_(nh), pnh_(pnh),
      ...
      kRoomSegmentation3FEnable_(false)
{
    ...
    pnh_.param("exploredAreaVoxelSize", explored_area_voxel_size_, explored_area_voxel_size_);
    pnh_.param("room_resolution", room_resolution_, room_resolution_);
    pnh_.param("rolling_occupancy_grid/resolution_x", occupancy_grid_resolution_, occupancy_grid_resolution_);
    pnh_.param("room_x", room_voxel_dimension_[0], room_voxel_dimension_[0]);
    pnh_.param("room_y", room_voxel_dimension_[1], room_voxel_dimension_[1]);
    pnh_.param("room_z", room_voxel_dimension_[2], room_voxel_dimension_[2]);
    pnh_.param("ceilingHeight_", ceiling_height_base_, ceiling_height_base_);
    pnh_.param("wall_thres_height_", wall_thres_height_base_, wall_thres_height_base_);
    pnh_.param("exploredAreaDisplayInterval", exploredAreaDisplayInterval_, exploredAreaDisplayInterval_);
    pnh_.param("dilation_iteration", dilation_iteration_, dilation_iteration_);
    pnh_.param("outward_distance_0", outward_distance_0_, outward_distance_0_);
    pnh_.param("outward_distance_1", outward_distance_1_, outward_distance_1_);
    pnh_.param("distance_threshold", distance_threshold_, distance_threshold_);
    pnh_.param("distance_angel_threshold", distance_angel_threshold_, distance_angel_threshold_);
    pnh_.param("angle_threshold_deg", angle_threshold_deg_, angle_threshold_deg_);
    pnh_.param("region_growing_radius", region_growing_radius_, region_growing_radius_);
    pnh_.param("min_room_size", min_room_size_, min_room_size_);
    pnh_.param("normal_search_num", normal_search_num_, normal_search_num_);
    pnh_.param("normal_search_radius", normal_search_radius_, normal_search_radius_);
    ...
    robot_position_ = geometry_msgs::Point();
    robot_position_.x = 0.0;
    robot_position_.y = 0.0;
    robot_position_.z = 0.0;

    // ==================== Create Subscriptions ====================
    sub_laser_cloud_ = nh_.subscribe<sensor_msgs::PointCloud2>("/livox/Pointcloud2", 20, &RoomSegmentationNode::laserCloudCallback, this);

    sub_state_estimation_ = nh_.subscribe<nav_msgs::Odometry>("/Odometry_gazebo", 20, &RoomSegmentationNode::stateEstimationCallback, this);

    sub_occupied_cloud_ = nh_.subscribe<sensor_msgs::PointCloud2>("/occupied_cloud", 20, &RoomSegmentationNode::occupiedCloudCallback, this);

    sub_freespace_cloud_ = nh_.subscribe<sensor_msgs::PointCloud2>("/freespace_cloud", 20, &RoomSegmentationNode::freespaceCloudCallback, this);
    ...
    pub_room_mask_ = nh_.advertise<sensor_msgs::Image>("/room_mask", 5);
    pub_door_cloud_ = nh_.advertise<sensor_msgs::PointCloud2>("/door_cloud", 5);
    pub_debug_ = nh_.advertise<sensor_msgs::PointCloud2>("/debug_cloud", 5);
    pub_debug_1_ = nh_.advertise<sensor_msgs::PointCloud2>("/free_cloud_1", 5);
    pub_room_boundary_ = nh_.advertise<geometry_msgs::PolygonStamped>("/current_room_boundary", 5);
    pub_room_boundary_tmp_ = nh_.advertise<geometry_msgs::PolygonStamped>("/navigation_boundary", 5);
    pub_polygon_ = nh_.advertise<visualization_msgs::MarkerArray>("/room_boundaries", 50);
    pub_wall_cloud_ = nh_.advertise<sensor_msgs::PointCloud2>("/walls", 5);
    pub_room_node_ = nh_.advertise<hybrid_planner::RoomNode>("/room_nodes", 50);
    pub_room_node_list_ = nh_.advertise<hybrid_planner::RoomNodeList>("/room_nodes_list", 5);
    pub_room_map_cloud_ = nh_.advertise<sensor_msgs::PointCloud2>("/room_map_cloud", 5);

    // ==================== Create Timer ====================
    timer_ = nh_.createTimer(ros::Duration(0.1), &RoomSegmentationNode::timerCallback, this);
...
void RoomSegmentationNode::saveImageToFile(const cv::Mat &image, const std::string &filename, bool force_save) {
    if (is_debug_ || force_save) {
        ...
        if (!flipped_image.empty()) {
            ...
            ROS_ERROR("Image is empty, cannot save: %s", filename.c_str());
        }
    }
}
...
geometry_msgs::PolygonStamped RoomSegmentationNode::computePolygonFromMaskCropped(const cv::Mat &mask) {
    ...
    geometry_msgs::PolygonStamped boundary_polygon;
...
void RoomSegmentationNode::stateEstimationCallback(const nav_msgs::Odometry::ConstPtr& msg) {
    robot_position_ = msg->pose.pose.position;
}

// ==================== Keyboard Input Callback ====================
void RoomSegmentationNode::keyboardInputCallback(const std_msgs::String::ConstPtr& msg) {
    if (msg->data == "demo" && !demo_frozen_) {
        demo_frozen_ = true;
        demo_publish_count_ = 0;
        ROS_INFO("Demo mode enabled: room segmentation frozen, republishing cached results.");
    }
    else if (msg->data == "resume" && demo_frozen_) {
        demo_frozen_ = false;
        demo_publish_count_ = 0;
        ROS_INFO("Demo mode disabled: room segmentation resumed.");
    }
}
    ...
    *explored_area_cloud_tmp_ += *laser_cloud_;
    ...
    if (explored_area_display_count_ >= 5 * exploredAreaDisplayInterval_)
    {
        ...
        *downsampled_ceiling_cloud_ += *downsampled_ceiling_cloud_tmp_;
        ...
        sensor_msgs::PointCloud2 explored_area_msg;
        ...
        sensor_msgs::PointCloud2 in_range_cloud_msg;
...
void RoomSegmentationNode::occupiedCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& msg) {
    ...
    {
        ROS_WARN("Occupied cloud is empty after height filtering.");
        return;
    }
...
void RoomSegmentationNode::freespaceCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
    if (demo_frozen_) {
        return;
    }
    ...
    {
        ROS_WARN("Freespace cloud is empty.");
        return;
    }
...
cv::Mat RoomSegmentationNode::getWall(const pcl::PointCloud<pcl::PointXYZINormal>::Ptr &cloud) {
    ...
    for (const auto &plane : plane_infos_)
    {
        ...
        for (const auto& pt : plane.cloud->points)
        {
            ...
            colored_pt.g = g;
            colored_pt.b = b;
            merged_cloud->points.push_back(colored_pt);
        }
    }
...
void RoomSegmentationNode::updateStateVoxel() {
    ...
    if (!updated_voxel_cloud_->empty())
    {
        ...
        sensor_msgs::PointCloud2 occupied_cloud_msg;
        pcl::toROSMsg(*updated_voxel_cloud_, occupied_cloud_msg);
        occupied_cloud_msg.header.stamp = ros::Time::now();
...
void RoomSegmentationNode::updateFreespace(pcl::PointCloud<pcl::PointXYZI>::Ptr &freespace_cloud_tmp)
{
    ...
    sensor_msgs::PointCloud2 freespace_cloud_msg;
    pcl::toROSMsg(*freespace_cloud_tmp, freespace_cloud_msg);
    freespace_cloud_msg.header.stamp = ros::Time::now();
...
void RoomSegmentationNode::updateRooms(cv::Mat &room_mask_cropped, cv::Mat &room_mask_new,
    ...
    {   
            ...
            values_set_old.find(value_old) == values_set_old.end())
        {
            ...
            ROS_INFO("Room %d: all_zero = %d, zero_count = %d, value_found = %d.",
    ...
    for (auto &id_room_node_pair : room_nodes_map_)
    {
        ...
        geometry_msgs::PolygonStamped new_room_polygon = computePolygonFromMaskCropped(mask_new);
...
void RoomSegmentationNode::roomSegmentation()
{
    ...
    if ((current_room_label > 0 && current_room_label <= static_cast<int>(found_region_masks.size())))
    {
        ...
        geometry_msgs::PolygonStamped boundary_polygon = computePolygonFromMaskCropped(current_room_mask);
    ...
    for (int label = 1; label < num_labels; ++label)
    {
        ...
        if (neighborLabels.size() != 2)
        {
            ROS_ERROR("Door mask has more than two labels or less than two labels, skipping this door.");
            continue;
        }
...
void RoomSegmentationNode::publishRoomNodes() {
    hybrid_planner::RoomNodeList room_node_list_msg;
    room_node_list_msg.header.stamp = ros::Time::now();
    ...
    for (const auto &id_room_node_pair : room_nodes_map_) {
        ...
        hybrid_planner::RoomNode room_node_msg;
        ...
        room_node_msg.room_mask = *cv_bridge::CvImage(std_msgs::Header(), "mono8", room_node.room_mask_).toImageMsg();
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "room_segmentation_node");
    ros::NodeHandle nh;
    ros::NodeHandle pnh("~");
    room_segmentation::RoomSegmentationNode node(nh, pnh);
    ROS_INFO("Room Segmentation Node is running...");
    ros::spin();
    return 0;
}