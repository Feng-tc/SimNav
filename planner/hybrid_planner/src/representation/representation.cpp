...
namespace representation_ns {
    ViewPointRep::ViewPointRep(int id, double x, double y, double z, pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr &cloud, pcl::PointCloud<pcl::PointXYZI>::Ptr &covered_cloud, const ros::Time &timestamp)
        : position_(), room_id_(-1)
    {
        ...
        position_.y = y;
        position_.z = z;
        cloud_.reset(new pcl::PointCloud<pcl::PointXYZRGBNormal>(*cloud));
        covered_cloud_.reset(new pcl::PointCloud<pcl::PointXYZI>(*covered_cloud));
        timestamp_ = timestamp;

        room_id_ = -1; // Initialize room ID to -1 (not assigned)
    }

    ViewPointRep::ViewPointRep(int id, const geometry_msgs::Point &position, pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr &cloud, pcl::PointCloud<pcl::PointXYZI>::Ptr &covered_cloud, const ros::Time &timestamp)
        ...
        : id_(id), show_id_(0), points_(points), alive(true), centroid_(0.0, 0.0, 0.0), area_(0.0), last_area_(0.0), is_visited_(false), is_covered_(false), is_labeled_(false), is_asked_(2), voxel_num_(0), is_connected_(false)
    {
        ...
        anchor_point_ = geometry_msgs::Point();
    }

    void RoomNodeRep::UpdateRoomNode(const std::vector<cv::Point> &points, const geometry_msgs::PolygonStamped &polygon)
    {
        points_ = points;
        polygon_ = polygon;
    ...
    void RoomNodeRep::UpdatePolygon(const geometry_msgs::PolygonStamped &polygon)
    {
        polygon_ = polygon;
        alive = true; // Mark the room as alive
    }

    bool RoomNodeRep::InRoom(const geometry_msgs::Point &point)
    {
    ...
    void RoomNodeRep::UpdateRoomNode(const hybrid_planner::RoomNode msg)
    {
        id_ = msg.id;
    ...
    ObjectNodeRep::ObjectNodeRep(const hybrid_planner::ObjectNode::ConstPtr msg)
    : room_id_(-1)
    {
    ...
    void ObjectNodeRep::UpdateObjectNode(const hybrid_planner::ObjectNode::ConstPtr msg)
    {
        object_id_ = msg->object_id; 
        label_ = msg->label;
        position_ = msg->position;
    ...
    Representation::Representation(ros::NodeHandle* nh, std::string world_frame_id)
    : nh_(nh)
    {
        viewpoint_rep_vis_cloud_.reset(new pcl::PointCloud<pcl::PointXYZ>());
        viewpoint_reps_ = std::vector<ViewPointRep>();
        room_nodes_map_ = std::map<int, RoomNodeRep>();
        object_node_rep_map_ = std::unordered_map<int, ObjectNodeRep>();
        latest_object_node_indices_ = std::set<int>();
        covered_points_all_.reset(new pcl::PointCloud<pcl::PointXYZI>());
    }

    void Representation::AddViewPointRepNode(const geometry_msgs::Point &position, pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr &cloud, pcl::PointCloud<pcl::PointXYZI>::Ptr &covered_cloud, const ros::Time &timestamp)
    {   
        ...
        // translate the viewpoint_rep to the visualization cloud (from geometry_msgs::Point to pcl::PointXYZ)
        pcl::PointXYZ point;
        point.x = position.x;
        point.y = position.y;
        point.z = position.z;
    ...
    int Representation::AddViewPointRep(const geometry_msgs::Point &position, pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr &cloud, pcl::PointCloud<pcl::PointXYZI>::Ptr &covered_cloud, const ros::Time &timestamp)
    {
        if (viewpoint_reps_.size() == 0){
            ...
            {
                double dist = misc_utils_ns::PointXYZDist<geometry_msgs::Point, geometry_msgs::Point>(viewpoint_reps_[i].GetPosition(), position);
                if (dist < min_dist)
                {
                    min_dist = dist;
                    min_dist_ind = i;
    ...
    void Representation::UpdateViewpointRoomIdsFromMask(const cv::Mat& room_mask, const Eigen::Vector3f& shift, float room_resolution) 
    {
        for (auto& viewpoint : viewpoint_reps_) {
            ...
            if (voxel.x() >= 0 && voxel.x() < room_mask.rows && 
                ...
                ROS_WARN("Viewpoint at (%.2f, %.2f, %.2f) is out of room mask bounds! Voxel: (%d, %d), Mask size: (%d, %d)", 
    ...
    void Representation::UpdateObjectNode(const hybrid_planner::ObjectNode::ConstPtr msg)
    {
        if (msg->object_id.size() == 0) {
            ROS_WARN("Received ObjectNode message with empty object_id");
            return;
        }
    ...
    geometry_msgs::Point Representation::GetViewPointRepNodePos(int index) const
    {
        if (index < 0 || index >= viewpoint_reps_.size()) {
            throw std::out_of_range("Viewpoint index out of range");
        }
    ...
    RoomNodeRep& Representation::AddRoomNode(const hybrid_planner::RoomNode& msg)
    {
    ...
    void Representation::SetObjectRoomRelation(int object_id, int new_room_id)
    {
        if (object_node_rep_map_.find(object_id) == object_node_rep_map_.end()) {
            ROS_ERROR("Object ID %d not found in object_node_rep_map_", object_id);
            return;
        }
        if (room_nodes_map_.find(new_room_id) == room_nodes_map_.end()) {
            ROS_ERROR("Room ID %d not found in room_nodes_map_", new_room_id);
            return;
        }
    ...
    void Representation::SetViewpointRoomRelation(int viewpoint_id, int new_room_id)
    {
        if (viewpoint_id < 0 || viewpoint_id >= viewpoint_reps_.size()) {
            ROS_ERROR("Viewpoint ID %d is out of bounds for viewpoint_reps_", viewpoint_id);
            return;
        }
        if (room_nodes_map_.find(new_room_id) == room_nodes_map_.end()) {
            ROS_ERROR("Room ID %d is out of bounds for room_nodes_", new_room_id);
            return;
        }
