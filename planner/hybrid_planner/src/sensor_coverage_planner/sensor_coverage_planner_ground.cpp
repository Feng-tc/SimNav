...
void SensorCoveragePlanner3D::ReadParameters() {
  ...
  private_nh_.param("kPhaseArrivalDist", kPhaseArrivalDist, 1.0);
  private_nh_.param("kMapClearingDist", kMapClearingDist, 8.0);
  ...
  private_nh_.param("kPhase1Enable", kPhase1Enable, true);
  private_nh_.param("kPhase2Enable", kPhase2Enable, true);
  private_nh_.param("kPhase3Enable", kPhase3Enable, true);
  private_nh_.param("kPhase4Enable", kPhase4Enable, true);
  private_nh_.param("kPhase5Enable", kPhase5Enable, true);
  private_nh_.param("kPhase6Enable", kPhase6Enable, true);

  private_nh_.param("kUsePhase4", kUsePhase4, false);
  private_nh_.param("kPhase4NoRoomCycles", kPhase4NoRoomCycles, 3);
  private_nh_.param<std::string>("kPhase4ElevatorId", kPhase4ElevatorId, "elevator_main");
  std::vector<double> phase4_1_waypoint_param;
  if (private_nh_.getParam("kPhase4_1Waypoint", phase4_1_waypoint_param) &&
      phase4_1_waypoint_param.size() >= 3) {
    kPhase4_1WaypointX = phase4_1_waypoint_param[0];
    kPhase4_1WaypointY = phase4_1_waypoint_param[1];
    kPhase4_1WaypointZ = phase4_1_waypoint_param[2];
  } else {
    private_nh_.param("kPhase4_1WaypointX", kPhase4_1WaypointX, 0.0);
    private_nh_.param("kPhase4_1WaypointY", kPhase4_1WaypointY, 2.5);
    private_nh_.param("kPhase4_1WaypointZ", kPhase4_1WaypointZ, 0.6);
    if (private_nh_.hasParam("kPhase4_1Waypoint")) {
      ROS_WARN("kPhase4_1Waypoint must have at least 3 elements [x, y, z]; using X/Y/Z fallback");
    }
  }
  std::vector<double> phase4_2_waypoint_param;
  if (private_nh_.getParam("kPhase4_2Waypoint", phase4_2_waypoint_param) &&
      phase4_2_waypoint_param.size() >= 3) {
    kPhase4_2WaypointX = phase4_2_waypoint_param[0];
    kPhase4_2WaypointY = phase4_2_waypoint_param[1];
    kPhase4_2WaypointZ = phase4_2_waypoint_param[2];
  } else {
    private_nh_.param("kPhase4_2WaypointX", kPhase4_2WaypointX, 2.0);
    private_nh_.param("kPhase4_2WaypointY", kPhase4_2WaypointY, 2.5);
    private_nh_.param("kPhase4_2WaypointZ", kPhase4_2WaypointZ, 0.6);
    if (private_nh_.hasParam("kPhase4_2Waypoint")) {
      ROS_WARN("kPhase4_2Waypoint must have at least 3 elements [x, y, z]; using X/Y/Z fallback");
    }
  }
  std::vector<double> phase4_3_waypoint_param;
  if (private_nh_.getParam("kPhase4_3Waypoint", phase4_3_waypoint_param) &&
      phase4_3_waypoint_param.size() >= 3) {
    kPhase4_3WaypointX = phase4_3_waypoint_param[0];
    kPhase4_3WaypointY = phase4_3_waypoint_param[1];
    kPhase4_3WaypointZ = phase4_3_waypoint_param[2];
  } else {
    private_nh_.param("kPhase4_3WaypointX", kPhase4_3WaypointX, 0.0);
    private_nh_.param("kPhase4_3WaypointY", kPhase4_3WaypointY, 2.5);
    private_nh_.param("kPhase4_3WaypointZ", kPhase4_3WaypointZ, 3.2);
    if (private_nh_.hasParam("kPhase4_3Waypoint")) {
      ROS_WARN("kPhase4_3Waypoint must have at least 3 elements [x, y, z]; using X/Y/Z fallback");
    }
  }
  if (kPhase4NoRoomCycles < 1) {
    ROS_WARN("kPhase4NoRoomCycles must be >= 1, resetting to 1");
    kPhase4NoRoomCycles = 1;
  }

  private_nh_.param("kUsePhase5Waypoint", kUsePhase5Waypoint, false);
  std::vector<double> phase5_waypoint_param;
  if (private_nh_.getParam("kPhase5Waypoint", phase5_waypoint_param) &&
      phase5_waypoint_param.size() >= 3) {
    kPhase5WaypointX = phase5_waypoint_param[0];
    kPhase5WaypointY = phase5_waypoint_param[1];
    kPhase5WaypointZ = phase5_waypoint_param[2];
  } else {
    private_nh_.param("kPhase5WaypointX", kPhase5WaypointX, 0.0);
    private_nh_.param("kPhase5WaypointY", kPhase5WaypointY, 30.0);
    private_nh_.param("kPhase5WaypointZ", kPhase5WaypointZ, 3.0);
    if (private_nh_.hasParam("kPhase5Waypoint")) {
      ROS_WARN("kPhase5Waypoint must have at least 3 elements [x, y, z]; using X/Y/Z fallback");
    }
  }

  private_nh_.param("kPhase7Enable", kPhase7Enable, true);
  ...
  private_nh_.param("kRoomBoundaryInset", kRoomBoundaryInset, 0.5);
  if (kRoomBoundaryInset < 0.0) {
    ROS_WARN("kRoomBoundaryInset must be >= 0, resetting to 0");
    kRoomBoundaryInset = 0.0;
  }
  ...
  private_nh_.param("kPhase7ElevatorTargetFloor", kPhase7ElevatorTargetFloor, 2);
  std::vector<double> phase7_1_waypoint_param;
  if (private_nh_.getParam("kPhase7_1Waypoint", phase7_1_waypoint_param) &&
      phase7_1_waypoint_param.size() >= 3) {
    kPhase7_1WaypointX = phase7_1_waypoint_param[0];
    kPhase7_1WaypointY = phase7_1_waypoint_param[1];
    kPhase7_1WaypointZ = phase7_1_waypoint_param[2];
  } else {
    private_nh_.param("kPhase7_1WaypointX", kPhase7_1WaypointX, 0.0);
    private_nh_.param("kPhase7_1WaypointY", kPhase7_1WaypointY, 2.5);
    private_nh_.param("kPhase7_1WaypointZ", kPhase7_1WaypointZ, 2.9);
    if (private_nh_.hasParam("kPhase7_1Waypoint")) {
      ROS_WARN("kPhase7_1Waypoint must have at least 3 elements [x, y, z]; using X/Y/Z fallback");
    }
  }
  std::vector<double> phase7_2_waypoint_param;
  if (private_nh_.getParam("kPhase7_2Waypoint", phase7_2_waypoint_param) &&
      phase7_2_waypoint_param.size() >= 3) {
    kPhase7_2WaypointX = phase7_2_waypoint_param[0];
    kPhase7_2WaypointY = phase7_2_waypoint_param[1];
    kPhase7_2WaypointZ = phase7_2_waypoint_param[2];
  } else {
    private_nh_.param("kPhase7_2WaypointX", kPhase7_2WaypointX, 2.2);
    private_nh_.param("kPhase7_2WaypointY", kPhase7_2WaypointY, 2.5);
    private_nh_.param("kPhase7_2WaypointZ", kPhase7_2WaypointZ, 2.9);
    if (private_nh_.hasParam("kPhase7_2Waypoint")) {
      ROS_WARN("kPhase7_2Waypoint must have at least 3 elements [x, y, z]; using X/Y/Z fallback");
    }
  }
  std::vector<double> phase7_3_waypoint_param;
  if (private_nh_.getParam("kPhase7_3Waypoint", phase7_3_waypoint_param) &&
      phase7_3_waypoint_param.size() >= 3) {
    kPhase7_3WaypointX = phase7_3_waypoint_param[0];
    kPhase7_3WaypointY = phase7_3_waypoint_param[1];
    kPhase7_3WaypointZ = phase7_3_waypoint_param[2];
  } else {
    private_nh_.param("kPhase7_3WaypointX", kPhase7_3WaypointX, 0.0);
    private_nh_.param("kPhase7_3WaypointY", kPhase7_3WaypointY, 2.5);
    private_nh_.param("kPhase7_3WaypointZ", kPhase7_3WaypointZ, 5.2);
    if (private_nh_.hasParam("kPhase7_3Waypoint")) {
      ROS_WARN("kPhase7_3Waypoint must have at least 3 elements [x, y, z]; using X/Y/Z fallback");
    }
  }
  if (kPhase7NoRoomCycles < 1) {
    ROS_WARN("kPhase7NoRoomCycles must be >= 1, resetting to 1");
    kPhase7NoRoomCycles = 1;
  }

  private_nh_.param("kUsePhase8Waypoint", kUsePhase8Waypoint, false);
  std::vector<double> phase8_waypoint_param;
  if (private_nh_.getParam("kPhase8Waypoint", phase8_waypoint_param) &&
      phase8_waypoint_param.size() >= 3) {
    kPhase8WaypointX = phase8_waypoint_param[0];
    kPhase8WaypointY = phase8_waypoint_param[1];
    kPhase8WaypointZ = phase8_waypoint_param[2];
  } else {
    private_nh_.param("kPhase8WaypointX", kPhase8WaypointX, 0.0);
    private_nh_.param("kPhase8WaypointY", kPhase8WaypointY, 30.0);
    private_nh_.param("kPhase8WaypointZ", kPhase8WaypointZ, 5.2);
    if (private_nh_.hasParam("kPhase8Waypoint")) {
      ROS_WARN("kPhase8Waypoint must have at least 3 elements [x, y, z]; using X/Y/Z fallback");
    }
  }

  kUsePhase2Waypoint = kUsePhase2Waypoint && kPhase2Enable;
  kUsePhase4         = kUsePhase4 && kPhase4Enable;
  kUsePhase5Waypoint = kUsePhase5Waypoint && kPhase5Enable;
  kUsePhase7         = kUsePhase7 && kPhase7Enable;
  kUsePhase8Waypoint = kUsePhase8Waypoint && kPhase8Enable;
  ...
  LoadElevatorConfig();
  LoadRoomTasks();
  ROS_INFO("Configured-room mission loaded %zu room tasks",
           room_tasks_.size());
}

double SensorCoveragePlanner3D::XmlRpcToDouble(
...
bool SensorCoveragePlanner3D::LoadPointParam(
    const ros::NodeHandle& nh, const std::string& key,
    geometry_msgs::Point* point, double default_x, double default_y,
    double default_z) {
  std::vector<double> values;
  if (nh.getParam(key, values) && values.size() >= 3) {
    point->x = values[0];
    point->y = values[1];
    point->z = values[2];
    return true;
...
void SensorCoveragePlanner3D::LoadElevatorConfig() {
  private_nh_.param<std::string>("kElevatorId", kElevatorId,
                                 "elevator_main");
  private_nh_.param("kElevatorServedFloorCount",
                    kElevatorServedFloorCount, 3);

  LoadPointParam(private_nh_, "kElevator1_1Waypoint",
                 &elevator_trips_[0].outside, 0.0, 2.5, 0.6);
  LoadPointParam(private_nh_, "kElevator1_2Waypoint",
                 &elevator_trips_[0].inside, 2.3, 2.5, 0.6);
  LoadPointParam(private_nh_, "kElevator1_3Waypoint",
                 &elevator_trips_[0].exit, 0.0, 2.5, 3.2);
  private_nh_.param("kElevator1TargetFloor",
                    elevator_trips_[0].target_floor, 1);
  private_nh_.param("kElevator1WaitFloor",
                    elevator_trips_[0].wait_floor, 0);

  LoadPointParam(private_nh_, "kElevator2_1Waypoint",
                 &elevator_trips_[1].outside, 0.0, 2.5, 3.2);
  LoadPointParam(private_nh_, "kElevator2_2Waypoint",
                 &elevator_trips_[1].inside, 2.3, 2.5, 3.2);
  LoadPointParam(private_nh_, "kElevator2_3Waypoint",
                 &elevator_trips_[1].exit, 0.0, 2.5, 5.8);
  private_nh_.param("kElevator2TargetFloor",
                    elevator_trips_[1].target_floor, 2);
  private_nh_.param("kElevator2WaitFloor",
                    elevator_trips_[1].wait_floor, 1);
}

void SensorCoveragePlanner3D::LoadRoomTasks() {
  room_tasks_.clear();
  XmlRpc::XmlRpcValue tasks;
  if (!private_nh_.getParam("kRoomTasks", tasks)) {
    ROS_WARN("No kRoomTasks parameter found; mission has no room tasks");
    return;
  }
  if (tasks.getType() != XmlRpc::XmlRpcValue::TypeArray) {
    ROS_ERROR("kRoomTasks must be an array");
    return;
  }

  for (int i = 0; i < tasks.size(); ++i) {
    XmlRpc::XmlRpcValue& value = tasks[i];
    if (value.getType() != XmlRpc::XmlRpcValue::TypeStruct ||
        !value.hasMember("floor") ||
        !value.hasMember("entry_waypoints") ||
        !value.hasMember("bbox")) {
      ROS_ERROR("kRoomTasks[%d] must contain floor, entry_waypoints, and bbox",
                i);
      continue;
    }

    ConfiguredRoomTask task;
    task.floor = static_cast<int>(value["floor"]);
    XmlRpc::XmlRpcValue& entries = value["entry_waypoints"];
    if (entries.getType() == XmlRpc::XmlRpcValue::TypeArray) {
      for (int j = 0; j < entries.size(); ++j) {
        if (entries[j].getType() != XmlRpc::XmlRpcValue::TypeArray ||
            entries[j].size() < 3) {
          continue;
        }
        geometry_msgs::Point point;
        point.x = XmlRpcToDouble(entries[j][0]);
        point.y = XmlRpcToDouble(entries[j][1]);
        point.z = XmlRpcToDouble(entries[j][2]);
        task.entry_waypoints.push_back(point);
      }
    }
    if (task.entry_waypoints.empty()) {
      ROS_ERROR("kRoomTasks[%d] has no valid entry waypoints", i);
      continue;
    }

    if (value.hasMember("exit_waypoints")) {
      XmlRpc::XmlRpcValue& exits = value["exit_waypoints"];
      if (exits.getType() == XmlRpc::XmlRpcValue::TypeArray) {
        for (int j = 0; j < exits.size(); ++j) {
          if (exits[j].getType() != XmlRpc::XmlRpcValue::TypeArray ||
              exits[j].size() < 3) {
            continue;
          }
          geometry_msgs::Point point;
          point.x = XmlRpcToDouble(exits[j][0]);
          point.y = XmlRpcToDouble(exits[j][1]);
          point.z = XmlRpcToDouble(exits[j][2]);
          task.exit_waypoints.push_back(point);
        }
      }
    }

    XmlRpc::XmlRpcValue& bbox = value["bbox"];
    if (bbox.getType() != XmlRpc::XmlRpcValue::TypeArray ||
        bbox.size() < 4) {
      ROS_ERROR("kRoomTasks[%d] bbox must be [xmin,xmax,ymin,ymax]", i);
      continue;
    }
    for (int j = 0; j < 4; ++j) {
      task.bbox[j] = XmlRpcToDouble(bbox[j]);
    }
    if (task.bbox[0] >= task.bbox[1] ||
        task.bbox[2] >= task.bbox[3]) {
      ROS_ERROR("kRoomTasks[%d] has an invalid bbox", i);
      continue;
    }

    if (value.hasMember("explored_threshold")) {
      task.explored_threshold =
          static_cast<int>(value["explored_threshold"]);
    }
    if (value.hasMember("timeout")) {
      task.timeout = XmlRpcToDouble(value["timeout"]);
    }
    if (task.explored_threshold < 0 || task.explored_threshold > 100 ||
        task.timeout <= 0.0) {
      ROS_ERROR(
          "kRoomTasks[%d] explored_threshold must be in [0, 100] and timeout "
          "must be positive",
          i);
      continue;
    }
    room_tasks_.push_back(task);
  }
}
...
void SensorCoveragePlanner3D::InitializeData() {
  ...
  viewpoint_rep_msg_ = hybrid_planner::ViewpointRep();
  ...
  phase4_waypoint_.x = 0.0;
  phase4_waypoint_.y = 0.0;
  phase4_waypoint_.z = 0.0;
  phase5_waypoint_.x = 0.0;
  phase5_waypoint_.y = 0.0;
  phase5_waypoint_.z = 0.0;
  phase1_door_opened_ = false;
  phase1_pointcloud_reset_ = false;
  phase5_arrived_ = false;
  phase4_elevator_ready_ = false;
  phase4_step_ = 0;
  ...
  mission_mode_ = MissionMode::PHASE1_DOOR;
  current_room_task_index_ = 0;
  current_floor_ = 0;
  room_exiting_ = false;
  exit_room_task_index_ = -1;
  entry_waypoint_index_ = 0;
  exit_waypoint_index_ = 0;
  room_no_exploring_cells_ = false;
  room_has_valid_coverage_update_ = false;
  elevator_trip_index_ = 0;
  elevator_step_ = 0;
...
SensorCoveragePlanner3D::SensorCoveragePlanner3D(ros::NodeHandle& nh, ros::NodeHandle& private_nh)
    : nh_(nh), private_nh_(private_nh), keypose_cloud_update_(false),
      ...
      phase5_arrived_(false),
      phase4_elevator_ready_(false), phase4_step_(0), phase4_ride_requested_(false),
      no_unexplored_room_counter_(0),
...
bool SensorCoveragePlanner3D::initialize() {
  ...
  set_door_state_client_ =
      nh_.serviceClient<building_generator_interfaces::SetDoorState>(
          "/set_door_state");
  call_elevator_client_ =
      nh_.serviceClient<building_generator_interfaces::CallElevator>(
          "/call_elevator");
  ROS_INFO("Configured-room mission: phase1_door=%d, rooms=%zu, arrival_dist=%.2f",
           kUsePhase1Door, room_tasks_.size(), kPhaseArrivalDist);
  InitializeData();
  if (!room_tasks_.empty()) {
    local_coverage_planner_->SetConfiguredRoomExploration(true);
  }
  ...
  object_node_list_sub_ = nh_.subscribe<hybrid_planner::ObjectNodeList>("/object_nodes_list", 20, &SensorCoveragePlanner3D::ObjectNodeListCallback, this);
  door_cloud_sub_ = nh_.subscribe<sensor_msgs::PointCloud2>("/door_cloud", 5, &SensorCoveragePlanner3D::DoorCloudCallback, this);
  room_node_list_sub_ = nh_.subscribe<hybrid_planner::RoomNodeList>("/room_nodes_list", 5, &SensorCoveragePlanner3D::RoomNodeListCallback, this);
  ...
  room_type_sub_ = nh_.subscribe<hybrid_planner::RoomType>("/room_type_answer", 10, &SensorCoveragePlanner3D::RoomTypeCallback, this);
  keyboard_input_sub_ = nh_.subscribe<std_msgs::String>("/keyboard_input", 5, &SensorCoveragePlanner3D::KeyboardInputCallback, this);
  target_object_instruction_sub_ = nh_.subscribe<hybrid_planner::TargetObjectInstruction>("/target_object_instruction", 5, &SensorCoveragePlanner3D::TargetObjectInstructionCallback, this);
  target_object_sub_ = nh_.subscribe<hybrid_planner::TargetObject>("/target_object_answer", 5, &SensorCoveragePlanner3D::TargetObjectCallback, this);
  anchor_object_sub_ = nh_.subscribe<hybrid_planner::TargetObject>("/anchor_object_answer", 5, &SensorCoveragePlanner3D::AnchorObjectCallback, this);
  ...
  reset_room_seg_pub_ = nh_.advertise<std_msgs::Bool>("/reset_room_segmentation", 1);
  ...
  viewpoint_rep_pub_ =
      nh_.advertise<hybrid_planner::ViewpointRep>("viewpoint_rep_header", 5);
  ...
  room_cloud_pub_ = nh_.advertise<sensor_msgs::PointCloud2>(
      "/room_cloud", 1);
  room_type_pub_ = nh_.advertise<hybrid_planner::RoomType>(
      "/room_type_query", 10);
  ...
  configured_mission_marker_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>(
          "/hybrid_planner/configured_mission", 1, true);
  target_object_pub_ = nh_.advertise<hybrid_planner::TargetObject>(
      "/target_object_query", 5);
  anchor_object_pub_ = nh_.advertise<hybrid_planner::TargetObject>(
      "/anchor_object_query", 5);
  target_object_spatial_pub_ = nh_.advertise<hybrid_planner::TargetObjectWithSpatial>(
...
void SensorCoveragePlanner3D::RegisteredScanCallback(
  ...
  *(registered_scan_stack_->cloud_) += *(registered_scan_tmp);
  ...
  if (registered_cloud_count_ == 0) {
    ...
    if (mission_mode_ == MissionMode::WAYPOINT_NAV) {
      planning_env_->UpdateRobotPosition(robot_position_);
      planning_env_->UpdateKeyposeCloudForCollisionCheck<PlannerCloudPointType>(
          keypose_cloud_->cloud_);
...
void SensorCoveragePlanner3D::ObjectNodeListCallback(
    const hybrid_planner::ObjectNodeList::ConstPtr& msg) {
  ...
  for (const auto& node : msg->nodes) {
    ...
    auto node_ptr = boost::make_shared<hybrid_planner::ObjectNode>(node);
...
void SensorCoveragePlanner3D::RoomNodeListCallback(
    const hybrid_planner::RoomNodeList::ConstPtr& room_node_list_msg)
{
  if (!initialized_)
  {
    return;
...
void SensorCoveragePlanner3D::RoomTypeCallback(
    const hybrid_planner::RoomType::ConstPtr& room_type_msg)
{
...
void SensorCoveragePlanner3D::TargetObjectInstructionCallback(
    const hybrid_planner::TargetObjectInstruction::ConstPtr& target_object_instruction_msg)
{
  if (!initialized_)
  {
    return;
...
void SensorCoveragePlanner3D::TargetObjectCallback(
    const hybrid_planner::TargetObject::ConstPtr& target_object_msg)
{
  if (!initialized_)
  {
    return;
...
void SensorCoveragePlanner3D::AnchorObjectCallback(
    const hybrid_planner::TargetObject::ConstPtr& anchor_object_msg)
{
  if (!initialized_)
  {
    return;
...
void SensorCoveragePlanner3D::SetStartAndEndRoomId()
{
  ...
  // Use the first hop on the BFS path for the current door segment.
  start_room_id_ = path[0] + 1; // +1 to convert to 1-based index
  end_room_id_ = path[1] + 1;   // +1 to convert to 1-based index
...
void SensorCoveragePlanner3D::SetCurrentRoomId()
{
  ...
  if (room_id_tmp_ != current_room_id_)
  {
    ...
    {
      // 机器人在走入该房间前就已经知道该房间是一个新房间，直接接受以避免卡死。
      ROS_INFO("Robot enters a wrong room %d, accepting it as current room.", room_id_tmp_);
      current_room_id_ = room_id_tmp_;
      robot_position_old_ = robot_position_;
      room_mask_old_ = room_mask_.clone();
      viewpoint_manager_->SetCurrentRoomId(current_room_id_);
...
void SensorCoveragePlanner3D::PublishPhase5Waypoint()
{
  geometry_msgs::PointStamped waypoint;
  waypoint.header.frame_id = kWorldFrameID;
  waypoint.header.stamp = ros::Time::now();
  waypoint.point = phase5_waypoint_;
  misc_utils_ns::Publish(&nh_, waypoint_pub_, waypoint, kWorldFrameID);
}

void SensorCoveragePlanner3D::PublishPhase4Waypoint()
{
  geometry_msgs::PointStamped waypoint;
  waypoint.header.frame_id = kWorldFrameID;
  waypoint.header.stamp = ros::Time::now();
  waypoint.point = phase4_waypoint_;
  misc_utils_ns::Publish(&nh_, waypoint_pub_, waypoint, kWorldFrameID);
}

void SensorCoveragePlanner3D::PublishPhase7Waypoint()
{
  geometry_msgs::PointStamped waypoint;
  waypoint.header.frame_id = kWorldFrameID;
  waypoint.header.stamp = ros::Time::now();
  waypoint.point = phase7_waypoint_;
  misc_utils_ns::Publish(&nh_, waypoint_pub_, waypoint, kWorldFrameID);
}

void SensorCoveragePlanner3D::PublishPhase8Waypoint()
{
  geometry_msgs::PointStamped waypoint;
  waypoint.header.frame_id = kWorldFrameID;
  waypoint.header.stamp = ros::Time::now();
  waypoint.point = phase8_waypoint_;
  misc_utils_ns::Publish(&nh_, waypoint_pub_, waypoint, kWorldFrameID);
}
...
void SensorCoveragePlanner3D::PublishPhase1Waypoint()
{
  if (!phase1_pointcloud_reset_) {
    ...
    clearing_msg.data = static_cast<float>(kMapClearingDist);
...
bool SensorCoveragePlanner3D::SetElevatorDoorById(const std::string& door_id, bool open,
                                                    double service_wait_timeout)
{
  ...
  srv.request.open = open;
  if (!set_door_state_client_.call(srv)) {
    ROS_WARN("set_door_state service call failed for elevator door \"%s\"", door_id.c_str());
    return false;
  }
  if (srv.response.accepted) {
    ROS_INFO("Elevator door \"%s\" %s", door_id.c_str(), open ? "opened" : "closed");
    return true;
  }
  ROS_WARN("Elevator door \"%s\" %s rejected: %s", door_id.c_str(),
           open ? "open" : "close", srv.response.message.c_str());
  return false;
}

bool SensorCoveragePlanner3D::SetElevatorDoor(bool open, double service_wait_timeout)
{
  return SetElevatorDoorById(kPhase4ElevatorDoorId, open, service_wait_timeout);
}

bool SensorCoveragePlanner3D::OpenAllElevatorDoors(double service_wait_timeout)
{
  for (int i = 0; i < kElevatorServedFloorCount; ++i) {
    const std::string door_id = "elevator_floor_" + std::to_string(i);
    if (!SetElevatorDoorById(door_id, true, service_wait_timeout)) {
      return false;
    }
  }
  ROS_INFO("Opened all %d elevator doors (elevator_floor_0 .. elevator_floor_%d)",
           kElevatorServedFloorCount, kElevatorServedFloorCount - 1);
  return true;
}

bool SensorCoveragePlanner3D::CallElevatorToFloor(int floor, bool open_doors,
                                                    double service_wait_timeout)
{
  ...
  srv.request.elevator_id = kElevatorId;
  ...
  if (srv.response.accepted) {
    ROS_INFO("Elevator \"%s\" called to floor %d (current_floor=%d, state=%s)",
             kElevatorId.c_str(), floor, srv.response.current_floor,
             srv.response.state.c_str());
    return true;
  }
  ROS_WARN("Elevator \"%s\" call to floor %d rejected: %s",
           kElevatorId.c_str(), floor, srv.response.message.c_str());
  return false;
}

bool SensorCoveragePlanner3D::TrySetupPhase4Elevator()
{
  if (phase4_elevator_ready_) {
    return true;
  }
  const bool doors_ok = OpenAllElevatorDoors(5.0);
  const bool elevator_ok = CallElevatorToFloor(0, true, 30.0);
  if (doors_ok && elevator_ok) {
    phase4_elevator_ready_ = true;
  }
  return phase4_elevator_ready_;
}

bool SensorCoveragePlanner3D::TrySetupPhase7Elevator()
{
  if (phase7_elevator_ready_) {
    return true;
  }
  const bool doors_ok = OpenAllElevatorDoors(5.0);
  const bool elevator_ok = CallElevatorToFloor(1, true, 30.0);
  if (doors_ok && elevator_ok) {
    phase7_elevator_ready_ = true;
  }
  return phase7_elevator_ready_;
}

bool SensorCoveragePlanner3D::TryOpenPhase1Entry(double service_wait_timeout)
{
  if (!SetMainEntranceDoor(true, service_wait_timeout)) {
    return false;
  }
  if (kUsePhase4 && !TrySetupPhase4Elevator()) {
    return false;
  }
  return true;
}

void SensorCoveragePlanner3D::InitPhase4()
{
  exploringPhase_ = 4;
  no_unexplored_room_counter_ = 0;
  phase4_ride_requested_ = false;
  ...
  phase4_waypoint_.x = kPhase4_1WaypointX;
  phase4_waypoint_.y = kPhase4_1WaypointY;
  phase4_waypoint_.z = kPhase4_1WaypointZ;

  if (phase4_elevator_ready_) {
    phase4_step_ = 1;
    ROS_INFO(
        "exploringPhase -> 4 (elevator ready from Phase1), step 1 wp1 [%.2f, %.2f, %.2f], "
        "wp2 [%.2f, %.2f, %.2f], wp3 [%.2f, %.2f, %.2f], target floor %d",
        kPhase4_1WaypointX, kPhase4_1WaypointY, kPhase4_1WaypointZ,
        kPhase4_2WaypointX, kPhase4_2WaypointY, kPhase4_2WaypointZ,
        kPhase4_3WaypointX, kPhase4_3WaypointY, kPhase4_3WaypointZ,
        kPhase4ElevatorTargetFloor);
    PublishPhase4Waypoint();
  } else {
    phase4_step_ = 0;
    ROS_INFO(
        "exploringPhase -> 4, wp1 [%.2f, %.2f, %.2f], wp2 [%.2f, %.2f, %.2f], "
...
void SensorCoveragePlanner3D::InitPhase5()
{
  ...
  phase5_waypoint_.z = kPhase5WaypointZ;
  ROS_INFO("exploringPhase -> 5, target [%.2f, %.2f, %.2f]",
           phase5_waypoint_.x, phase5_waypoint_.y, phase5_waypoint_.z);
  PublishPhase5Waypoint();
}

bool SensorCoveragePlanner3D::IsRoomExplorationPhase() const
{
  return mission_mode_ == MissionMode::ROOM_EXPLORATION;
}
...
void SensorCoveragePlanner3D::InitPhase6()
{
  ...
  if (robot_position_voxel.x() >= 0 &&
    ...
    const int room_id =
        room_mask_.at<int>(robot_position_voxel.x(), robot_position_voxel.y());
    if (room_id > 0) {
      current_room_id_ = room_id;
      if (representation_->HasRoomNode(current_room_id_)) {
        representation_->GetRoomNode(current_room_id_).SetIsVisited(true);
      }
      ROS_INFO("exploringPhase -> 6, accepted room %d at (%.2f, %.2f), starting 2F exploration",
               current_room_id_, robot_position_.x, robot_position_.y);
    } else {
      current_room_id_ = -1;
      ROS_WARN("exploringPhase -> 6, robot not in any room (mask id=%d), starting 2F exploration",
               room_id);
    }
  } else {
    current_room_id_ = -1;
    ROS_WARN("exploringPhase -> 6, robot position out of room mask bounds, starting 2F exploration");
  }
  ...
  if (!kPhase6Enable) {
    if (kPhase7Enable) {
      ...
      ROS_INFO("Phase6 disabled, marking exploration as finished.");
      exploration_finished_ = true;
    }
  }
}

void SensorCoveragePlanner3D::InitPhase7()
{
  exploringPhase_ = 7;
  no_unexplored_room_counter_ = 0;
  phase7_ride_requested_ = false;
  transit_across_room_ = false;

  phase7_waypoint_.x = kPhase7_1WaypointX;
  phase7_waypoint_.y = kPhase7_1WaypointY;
  phase7_waypoint_.z = kPhase7_1WaypointZ;

  if (phase7_elevator_ready_) {
    phase7_step_ = 1;
    ROS_INFO(
        "exploringPhase -> 7 (elevator ready), step 1 wp1 [%.2f, %.2f, %.2f], "
        "wp2 [%.2f, %.2f, %.2f], wp3 [%.2f, %.2f, %.2f], target floor %d",
        kPhase7_1WaypointX, kPhase7_1WaypointY, kPhase7_1WaypointZ,
        kPhase7_2WaypointX, kPhase7_2WaypointY, kPhase7_2WaypointZ,
        kPhase7_3WaypointX, kPhase7_3WaypointY, kPhase7_3WaypointZ,
        kPhase7ElevatorTargetFloor);
    PublishPhase7Waypoint();
  } else {
    phase7_step_ = 0;
    ROS_INFO(
        "exploringPhase -> 7, wp1 [%.2f, %.2f, %.2f], wp2 [%.2f, %.2f, %.2f], "
        "wp3 [%.2f, %.2f, %.2f], target floor %d",
        kPhase7_1WaypointX, kPhase7_1WaypointY, kPhase7_1WaypointZ,
        kPhase7_2WaypointX, kPhase7_2WaypointY, kPhase7_2WaypointZ,
        kPhase7_3WaypointX, kPhase7_3WaypointY, kPhase7_3WaypointZ,
...
void SensorCoveragePlanner3D::InitPhase8()
{
  ...
  phase8_waypoint_.z = kPhase8WaypointZ;
  ROS_INFO("exploringPhase -> 8, target [%.2f, %.2f, %.2f]",
           phase8_waypoint_.x, phase8_waypoint_.y, phase8_waypoint_.z);
  PublishPhase8Waypoint();
}
...
void SensorCoveragePlanner3D::InitPhase9()
{
  ...
  if (robot_position_voxel.x() >= 0 &&
    ...
    if (room_id > 0) {
      ...
      ROS_INFO("exploringPhase -> 9, accepted room %d at (%.2f, %.2f), starting 3F exploration",
               current_room_id_, robot_position_.x, robot_position_.y);
    } else {
      current_room_id_ = -1;
      ROS_WARN("exploringPhase -> 9, robot not in any room (mask id=%d), starting 3F exploration",
               room_id);
    }
  } else {
    current_room_id_ = -1;
    ROS_WARN("exploringPhase -> 9, robot position out of room mask bounds, starting 3F exploration");
  }
...
void SensorCoveragePlanner3D::InitPhase2()
{
  if (kUsePhase2Waypoint) {
    ...
    if (!kPhase3Enable) {
      ROS_INFO("Phase3 disabled, skipping to Phase4");
      InitPhase4();
    }
  }
}
  ...
  if (kUsePhase1Door) {
    ...
    if (TryOpenPhase1Entry(30.0)) {
      ...
      ROS_INFO("exploringPhase=1, entry open%s, target [%.2f, %.2f, %.2f]",
               kUsePhase4 ? " (main door + elevator prep)" : "",
...
void SensorCoveragePlanner3D::PublishWaypoint() {
  ...
  if (exploringPhase_ == 8) {
    PublishPhase8Waypoint();
    return;
  }
  if (exploringPhase_ == 7) {
    PublishPhase7Waypoint();
    return;
  }
  if (!IsRoomExplorationPhase()) {
  ...
  if (mission_mode_ == MissionMode::ROOM_EXPLORATION &&
      UsesConfiguredRoomMission() &&
      current_room_task_index_ >= 0 &&
      current_room_task_index_ < static_cast<int>(room_tasks_.size())) {
    ClipWaypointToRoomInterior(&waypoint.point,
                               room_tasks_[current_room_task_index_].bbox);
  }
...
void SensorCoveragePlanner3D::GetToRoomState(bool &at_room, bool &near_room_1, bool &near_room_2) 
{
  ...
  {
    at_room_ = (current_room_id_ == target_room_id_) && (target_room_id_ != -1);
    Eigen::Vector3d robot_position(robot_position_.x, robot_position_.y,
                                   robot_position_.z);
    double euler_length = (robot_position - door_position_).norm();
    ...
    ROS_INFO("!!!!!!!!!!!!!!Near room 1: %d, Near room 2: %d, At room: %d, Current room: %d, Next hop room: %d, Target room: %d",
                near_room_1_, near_room_2_, at_room_, current_room_id_, end_room_id_, target_room_id_);
  }
}
...
void SensorCoveragePlanner3D::PublishMissionWaypoint(
...
bool SensorCoveragePlanner3D::ArrivedAtMissionWaypoint(
    const geometry_msgs::Point& point) const {
  return misc_utils_ns::PointXYDist(robot_position_, point) <
         kPhaseArrivalDist;
}

bool SensorCoveragePlanner3D::WillUseElevator() const {
  for (const auto& task : room_tasks_) {
    if (task.floor != 0) {
      return true;
    }
  }
...
bool SensorCoveragePlanner3D::UsesConfiguredRoomMission() const {
  return !room_tasks_.empty();
}

geometry_msgs::Polygon SensorCoveragePlanner3D::MakeRoomBoundaryPolygon(
    const std::array<double, 4>& bbox, double z, double inset) {
  geometry_msgs::Polygon polygon;
  polygon.points.resize(4);
  double xmin = bbox[0] + inset;
  double xmax = bbox[1] - inset;
  double ymin = bbox[2] + inset;
  double ymax = bbox[3] - inset;
  ...
  polygon.points[0].x = xmin;
  polygon.points[0].y = ymin;
  polygon.points[0].z = z;
  polygon.points[1].x = xmax;
  polygon.points[1].y = ymin;
  polygon.points[1].z = z;
  polygon.points[2].x = xmax;
  polygon.points[2].y = ymax;
  polygon.points[2].z = z;
  polygon.points[3].x = xmin;
  polygon.points[3].y = ymax;
  polygon.points[3].z = z;
  return polygon;
}

bool SensorCoveragePlanner3D::ClipWaypointToRoomInterior(
    geometry_msgs::Point* waypoint,
    const std::array<double, 4>& bbox) const {
  const double inset = kRoomBoundaryInset;
  const double xmin = bbox[0] + inset;
  const double xmax = bbox[1] - inset;
  const double ymin = bbox[2] + inset;
  const double ymax = bbox[3] - inset;
  if (xmin >= xmax || ymin >= ymax) {
    return false;
  }
  if (waypoint->x >= xmin && waypoint->x <= xmax &&
      waypoint->y >= ymin && waypoint->y <= ymax) {
    return true;
  }

  double target_x = waypoint->x;
  double target_y = waypoint->y;
  for (int i = 0; i < 24; ++i) {
    target_x = robot_position_.x + (target_x - robot_position_.x) * 0.85;
    target_y = robot_position_.y + (target_y - robot_position_.y) * 0.85;
    if (target_x >= xmin && target_x <= xmax &&
        target_y >= ymin && target_y <= ymax) {
      waypoint->x = target_x;
      waypoint->y = target_y;
      return true;
    }
  }
  waypoint->x = robot_position_.x;
  waypoint->y = robot_position_.y;
  waypoint->z = robot_position_.z;
  return false;
}

void SensorCoveragePlanner3D::ResetConfiguredRoomContext() {
  current_room_id_ = -1;
  target_room_id_ = -1;
  start_room_id_ = -1;
  end_room_id_ = -1;
...
void SensorCoveragePlanner3D::PublishConfiguredMissionMarkers() {
  visualization_msgs::MarkerArray markers;
  visualization_msgs::Marker clear;
  clear.action = visualization_msgs::Marker::DELETEALL;
  markers.markers.push_back(clear);
  int id = 0;
  for (size_t room_index = 0; room_index < room_tasks_.size(); ++room_index) {
    const auto& task = room_tasks_[room_index];
    const double z = task.entry_waypoints.back().z;
    visualization_msgs::Marker boundary;
    boundary.header.frame_id = kWorldFrameID;
    boundary.header.stamp = ros::Time::now();
    boundary.ns = "configured_room_boundaries";
    boundary.id = id++;
    ...
    corner.z = z;
    corner.x = task.bbox[0]; corner.y = task.bbox[2];
    boundary.points.push_back(corner);
    corner.x = task.bbox[1]; corner.y = task.bbox[2];
    boundary.points.push_back(corner);
    corner.x = task.bbox[1]; corner.y = task.bbox[3];
    boundary.points.push_back(corner);
    corner.x = task.bbox[0]; corner.y = task.bbox[3];
    boundary.points.push_back(corner);
    boundary.points.push_back(boundary.points.front());
    markers.markers.push_back(boundary);

    const float kWaypointBlueR = 0.1f;
    const float kWaypointBlueG = 0.4f;
    const float kWaypointBlueB = 1.0f;
    const auto add_waypoint = [&](const geometry_msgs::Point& point,
                                  const std::string& marker_ns) {
      visualization_msgs::Marker marker;
      marker.header = boundary.header;
      marker.ns = marker_ns;
      marker.id = id++;
      ...
      marker.color.r = kWaypointBlueR;
      marker.color.g = kWaypointBlueG;
      marker.color.b = kWaypointBlueB;
      marker.color.a = 0.9;
      markers.markers.push_back(marker);
    };
    for (const auto& point : task.entry_waypoints) {
      add_waypoint(point, "configured_entry_waypoints");
    }
    for (const auto& point : task.exit_waypoints) {
      add_waypoint(point, "configured_exit_waypoints");
    }
  }

  const geometry_msgs::Point* current_target = nullptr;
  if (mission_mode_ == MissionMode::PHASE1_DOOR) {
    current_target = &phase1_waypoint_;
  } else if (mission_mode_ == MissionMode::WAYPOINT_NAV && room_exiting_ &&
             exit_room_task_index_ >= 0 &&
             exit_waypoint_index_ <
                 room_tasks_[exit_room_task_index_].exit_waypoints.size()) {
    current_target =
        &room_tasks_[exit_room_task_index_].exit_waypoints[exit_waypoint_index_];
  } else if (mission_mode_ == MissionMode::WAYPOINT_NAV &&
             current_room_task_index_ < static_cast<int>(room_tasks_.size()) &&
             entry_waypoint_index_ <
                 room_tasks_[current_room_task_index_].entry_waypoints.size()) {
    current_target =
        &room_tasks_[current_room_task_index_].entry_waypoints[entry_waypoint_index_];
  } else if (mission_mode_ == MissionMode::ELEVATOR_TRANSIT) {
    current_target = &elevator_waypoint_;
  }
  if (current_target != nullptr) {
    visualization_msgs::Marker target;
    target.header.frame_id = kWorldFrameID;
    target.header.stamp = ros::Time::now();
    target.ns = "configured_current_target";
    target.id = id++;
    target.type = visualization_msgs::Marker::SPHERE;
    target.action = visualization_msgs::Marker::ADD;
    target.pose.position = *current_target;
    target.pose.orientation.w = 1.0;
    target.scale.x = target.scale.y = target.scale.z = 0.65;
    target.color.r = 1.0;
    target.color.g = 0.0;
    target.color.b = 0.0;
    target.color.a = 1.0;
  ...
  configured_mission_marker_pub_.publish(markers);
}

void SensorCoveragePlanner3D::ApplyCurrentRoomBoundary() {
  const ConfiguredRoomTask& task = room_tasks_[current_room_task_index_];
  const double z = task.entry_waypoints.empty()
                       ? robot_position_.z
                       : task.entry_waypoints.back().z;
  const geometry_msgs::Polygon planning_polygon =
      MakeRoomBoundaryPolygon(task.bbox, z, kRoomBoundaryInset);
  viewpoint_manager_->UpdateViewPointBoundary(planning_polygon);
  planning_env_->UpdateCoverageBoundary(planning_polygon);
  grid_world_->SetPlanningBoundary(
      task.bbox[0] + kRoomBoundaryInset, task.bbox[1] - kRoomBoundaryInset,
      task.bbox[2] + kRoomBoundaryInset, task.bbox[3] - kRoomBoundaryInset);
}

void SensorCoveragePlanner3D::TransitionToWaypointNavigation() {
  if (current_room_task_index_ >= static_cast<int>(room_tasks_.size())) {
    FinishMission();
    return;
  }
  mission_mode_ = MissionMode::WAYPOINT_NAV;
  entry_waypoint_index_ = 0;
  exploringPhase_ = 10 + current_room_task_index_;
  PublishExploringPhase();
  const auto& waypoint =
      room_tasks_[current_room_task_index_].entry_waypoints.front();
  ROS_INFO(
      "exploringPhase=%d: navigate to room %d entry waypoint 1/%zu "
      "[%.2f, %.2f, %.2f]",
      exploringPhase_, current_room_task_index_,
      room_tasks_[current_room_task_index_].entry_waypoints.size(),
      waypoint.x, waypoint.y, waypoint.z);
  PublishMissionWaypoint(waypoint);
}

void SensorCoveragePlanner3D::TransitionToRoomExploration() {
  mission_mode_ = MissionMode::ROOM_EXPLORATION;
  exploringPhase_ = 100 + current_room_task_index_;
  PublishExploringPhase();
  room_exploration_start_time_ = ros::Time::now();
  room_no_exploring_cells_ = false;
  room_has_valid_coverage_update_ = false;
  last_status_frontier_ = -1;
  last_status_uncovered_ = -1;
  ...
  last_status_local_complete_ = false;
  no_unexplored_room_counter_ = 0;
  exploration_path_ = exploration_path_ns::ExplorationPath();
  lookahead_point_update_ = false;
  reset_waypoint_ = true;
  viewpoint_manager_->ResetViewPointCoverage();
  ResetConfiguredRoomContext();
  ApplyCurrentRoomBoundary();

  const auto& task = room_tasks_[current_room_task_index_];
  ROS_INFO(
      "exploringPhase=%d: start room %d coverage "
      "(bbox=[%.1f,%.1f,%.1f,%.1f])",
      exploringPhase_, current_room_task_index_, task.bbox[0], task.bbox[1],
      task.bbox[2], task.bbox[3]);
}

void SensorCoveragePlanner3D::TransitionToElevator(int target_floor) {
  int trip_index = -1;
  for (int i = 0; i < 2; ++i) {
    if (elevator_trips_[i].target_floor == target_floor) {
      trip_index = i;
      break;
    }
  }
  if (trip_index < 0) {
    ROS_ERROR("No kElevator1/2 trip targets floor %d; finishing mission",
              target_floor);
    FinishMission();
    return;
  }
  mission_mode_ = MissionMode::ELEVATOR_TRANSIT;
  elevator_trip_index_ = trip_index;
  elevator_step_ = 0;
  elevator_ride_requested_ = false;
  elevator_waypoint_ = elevator_trips_[trip_index].outside;
  exploringPhase_ = 200 + trip_index;
  PublishExploringPhase();
  ROS_INFO("exploringPhase=%d: start elevator transit to floor %d",
           exploringPhase_, target_floor);
}

bool SensorCoveragePlanner3D::TrySetupElevator(
    int trip_index, double door_wait, double elevator_wait) {
  return OpenAllElevatorDoors(door_wait) &&
         CallElevatorToFloor(elevator_trips_[trip_index].wait_floor, false,
                             elevator_wait);
}

void SensorCoveragePlanner3D::HandlePhase1Door() {
  if (!phase1_door_opened_) {
    if (!SetMainEntranceDoor(true, 5.0)) {
      ROS_WARN_THROTTLE(
          2.0, "exploringPhase=1 step 1: waiting for main entrance door");
      return;
    }
    phase1_door_opened_ = true;
    ROS_INFO("exploringPhase=1 step 2: entrance opened, goto [%.2f, %.2f, %.2f]",
             phase1_waypoint_.x, phase1_waypoint_.y, phase1_waypoint_.z);
  }
  PublishPhase1Waypoint();
  if (ArrivedAtMissionWaypoint(phase1_waypoint_)) {
    ROS_INFO("exploringPhase=1 step 3: building entry reached");
    if (!room_tasks_.empty() && room_tasks_.front().floor != current_floor_) {
      TransitionToElevator(room_tasks_.front().floor);
    } else {
      TransitionToWaypointNavigation();
    }
  }
}

void SensorCoveragePlanner3D::HandleWaypointNavigation() {
  if (room_exiting_) {
    const auto& task = room_tasks_[exit_room_task_index_];
    const auto& waypoint = task.exit_waypoints[exit_waypoint_index_];
    PublishMissionWaypoint(waypoint);
    if (!ArrivedAtMissionWaypoint(waypoint)) {
      return;
    }
    ++exit_waypoint_index_;
    ROS_INFO("exploringPhase=%d: room %d exit waypoint %zu/%zu reached",
             exploringPhase_, exit_room_task_index_, exit_waypoint_index_,
             task.exit_waypoints.size());
    if (exit_waypoint_index_ < task.exit_waypoints.size()) {
      return;
    }
    room_exiting_ = false;
    exit_waypoint_index_ = 0;
    if (current_room_task_index_ >= static_cast<int>(room_tasks_.size())) {
      FinishMission();
    } else if (room_tasks_[current_room_task_index_].floor != current_floor_) {
      TransitionToElevator(room_tasks_[current_room_task_index_].floor);
    } else {
      TransitionToWaypointNavigation();
    }
    return;
  ...
  const auto& task = room_tasks_[current_room_task_index_];
  const auto& waypoint = task.entry_waypoints[entry_waypoint_index_];
  PublishMissionWaypoint(waypoint);
  if (!ArrivedAtMissionWaypoint(waypoint)) {
    return;
  }
  ++entry_waypoint_index_;
  ROS_INFO("exploringPhase=%d: room %d entry waypoint %zu/%zu reached",
           exploringPhase_, current_room_task_index_, entry_waypoint_index_,
           task.entry_waypoints.size());
  if (entry_waypoint_index_ >= task.entry_waypoints.size()) {
    entry_waypoint_index_ = 0;
    TransitionToRoomExploration();
  }
}

void SensorCoveragePlanner3D::HandleElevatorTransit() {
  const auto& trip = elevator_trips_[elevator_trip_index_];
  switch (elevator_step_) {
    case 0:
      if (TrySetupElevator(elevator_trip_index_, 5.0, 30.0)) {
        elevator_step_ = 1;
        elevator_waypoint_ = trip.outside;
        ROS_INFO("exploringPhase=%d step 1: elevator ready", exploringPhase_);
      } else {
        ROS_WARN_THROTTLE(2.0, "exploringPhase=%d: waiting for elevator setup",
                          exploringPhase_);
      }
      break;
    case 1:
      PublishMissionWaypoint(trip.outside);
      if (ArrivedAtMissionWaypoint(trip.outside)) {
        elevator_step_ = 2;
        elevator_waypoint_ = trip.inside;
        ROS_INFO("exploringPhase=%d step 2: enter elevator", exploringPhase_);
      }
      break;
    case 2:
      PublishMissionWaypoint(trip.inside);
      if (ArrivedAtMissionWaypoint(trip.inside)) {
        elevator_step_ = 3;
        ROS_INFO("exploringPhase=%d step 3: call floor %d", exploringPhase_,
                 trip.target_floor);
      }
      break;
    case 3:
      ...
      if (!elevator_ride_requested_ &&
          CallElevatorToFloor(trip.target_floor, true, 30.0)) {
        elevator_ride_requested_ = true;
        elevator_step_ = 4;
      }
      break;
    case 4:
      if (robot_position_.z >= trip.exit.z - 0.5) {
        elevator_step_ = 5;
        elevator_waypoint_ = trip.exit;
        ROS_INFO("exploringPhase=%d step 5: elevator arrived", exploringPhase_);
      } else {
        ROS_INFO_THROTTLE(2.0, "exploringPhase=%d step 4: elevator moving",
                          exploringPhase_);
      }
      break;
    case 5:
      PublishMissionWaypoint(trip.exit);
      if (ArrivedAtMissionWaypoint(trip.exit)) {
        current_floor_ = trip.target_floor;
        elevator_ride_requested_ = false;
        ROS_INFO("exploringPhase=%d step 6: elevator exit complete",
                 exploringPhase_);
        TransitionToWaypointNavigation();
      }
      break;
...
void SensorCoveragePlanner3D::CompleteCurrentRoom(
    const std::string& reason) {
  const int completed_index = current_room_task_index_;
  const auto& task = room_tasks_[completed_index];
  const double elapsed =
      (ros::Time::now() - room_exploration_start_time_).toSec();
  ROS_INFO(
      "exploringPhase=%d: room %d complete: %s "
      "(no_exploring_cells=%d local_done=%d elapsed=%.1fs)",
      exploringPhase_, completed_index, reason.c_str(),
      room_no_exploring_cells_, last_status_local_complete_, elapsed);
  ++current_room_task_index_;
  if (!task.exit_waypoints.empty()) {
    room_exiting_ = true;
    exit_room_task_index_ = completed_index;
    exit_waypoint_index_ = 0;
    mission_mode_ = MissionMode::WAYPOINT_NAV;
    exploringPhase_ = 150 + completed_index;
    PublishExploringPhase();
    PublishMissionWaypoint(task.exit_waypoints.front());
  } else if (current_room_task_index_ >=
             static_cast<int>(room_tasks_.size())) {
    FinishMission();
  } else if (room_tasks_[current_room_task_index_].floor != current_floor_) {
    TransitionToElevator(room_tasks_[current_room_task_index_].floor);
  } else {
...
void SensorCoveragePlanner3D::FinishMission() {
  if (mission_mode_ == MissionMode::FINISHED) {
    return;
  }
  mission_mode_ = MissionMode::FINISHED;
  exploration_finished_ = true;
  std_msgs::Bool finished;
  finished.data = true;
  exploration_finish_pub_.publish(finished);
  ROS_INFO("Configured-room mission complete");
}

void SensorCoveragePlanner3D::RunCoveragePlanningCycle() {
  ...
  if (viewpoint_candidate_count == 0) {
    last_status_vp_count_ = 0;
    ROS_WARN("Cannot get candidate viewpoints, skipping this coverage update");
    return;
  }
  UpdateKeyposeGraph();
  ...
  room_has_valid_coverage_update_ = true;
  ...
  room_no_exploring_cells_ = grid_world_->IsReturningHome();
  ...
  PublishWaypoint();
  ROS_INFO(
      "room exploring | no_exploring_cells=%d local_done=%d",
      room_no_exploring_cells_, last_status_local_complete_);
  ...
  const Eigen::Vector3d viewpoint_origin = viewpoint_manager_->GetOrigin();
...
void SensorCoveragePlanner3D::execute() {
  ...
  if (!initialized_) {
    ...
    if (start_time_ == 0.0) {
      ROS_ERROR("Start time is zero; ROS time source is not ready");
      return;
    }
    ...
    if (room_tasks_.empty()) {
      FinishMission();
    } else if (kUsePhase1Door) {
      mission_mode_ = MissionMode::PHASE1_DOOR;
      exploringPhase_ = 1;
      PublishExploringPhase();
      ROS_INFO("exploringPhase=1 step 1: opening main entrance door \"%s\"",
               kMainEntranceDoorId.c_str());
    } else if (room_tasks_.front().floor != current_floor_) {
      TransitionToElevator(room_tasks_.front().floor);
    } else {
      TransitionToWaypointNavigation();
    }
    PublishConfiguredMissionMarkers();
    return;
  }

  PublishExploringPhase();
  switch (mission_mode_) {
    case MissionMode::PHASE1_DOOR:
      HandlePhase1Door();
      break;
    case MissionMode::WAYPOINT_NAV:
      if (keypose_cloud_update_) {
        keypose_cloud_update_ = false;
        planning_env_->UpdateRobotPosition(robot_position_);
        planning_env_->UpdateKeyposeCloudForCollisionCheck<PlannerCloudPointType>(
      ...
      break;
    case MissionMode::ROOM_EXPLORATION: {
      RunCoveragePlanningCycle();
      if (room_has_valid_coverage_update_ && room_no_exploring_cells_ &&
          last_status_local_complete_) {
        CompleteCurrentRoom("no exploring cells");
      }
      break;
    }
    case MissionMode::ELEVATOR_TRANSIT:
      HandleElevatorTransit();
      break;
...
#if 0  // Legacy Phase 1-9 dispatcher retained for reference; never compiled.
  PublishExploringPhase();
  Timer overall_processing_timer("overall processing");
  update_representation_runtime_ = 0;
  local_viewpoint_sampling_runtime_ = 0;
  ...
  if (!initialized_) {
    ...
    if (kPhase1Enable) {
      ...
      ROS_INFO("Phase1 disabled, skipping to Phase2");
      InitPhase2();
    }
    return;
  }
  ...
  if (exploringPhase_ == 2) {
    ...
    if (dist_to_phase2_waypoint < kPhaseArrivalDist) {
      exploringPhase_ = 3;
  ...
  if (exploringPhase_ == 4) {
    ...
    switch (phase4_step_) {
      case 0: {
        if (phase4_elevator_ready_) {
          phase4_step_ = 1;
          ROS_INFO("Phase4 step 1: approach outside waypoint [%.2f, %.2f, %.2f]",
                   phase4_waypoint_.x, phase4_waypoint_.y, phase4_waypoint_.z);
          PublishPhase4Waypoint();
          break;
        }
        if (TrySetupPhase4Elevator()) {
          phase4_step_ = 1;
          phase4_waypoint_.x = kPhase4_1WaypointX;
          phase4_waypoint_.y = kPhase4_1WaypointY;
          phase4_waypoint_.z = kPhase4_1WaypointZ;
          ROS_INFO("Phase4 step 1 (fallback setup): approach outside waypoint [%.2f, %.2f, %.2f]",
                   phase4_waypoint_.x, phase4_waypoint_.y, phase4_waypoint_.z);
          PublishPhase4Waypoint();
        } else {
          ROS_WARN_THROTTLE(2.0,
                            "Phase4 step 0 (fallback): waiting for elevator setup (all doors + floor 0)");
        }
        break;
      }
      case 1: {
        PublishPhase4Waypoint();
        const double dist_outside =
            misc_utils_ns::PointXYDist(robot_position_, phase4_waypoint_);
        if (dist_outside < kPhaseArrivalDist) {
          phase4_step_ = 2;
          phase4_waypoint_.x = kPhase4_2WaypointX;
          phase4_waypoint_.y = kPhase4_2WaypointY;
          phase4_waypoint_.z = kPhase4_2WaypointZ;
          ROS_INFO("Phase4 step 2: enter car waypoint [%.2f, %.2f, %.2f]",
                   phase4_waypoint_.x, phase4_waypoint_.y, phase4_waypoint_.z);
        }
        break;
      }
      case 2: {
        PublishPhase4Waypoint();
        const double dist_inside =
            misc_utils_ns::PointXYDist(robot_position_, phase4_waypoint_);
        if (dist_inside < kPhaseArrivalDist) {
          phase4_step_ = 3;
          ROS_INFO("Phase4 step 3: calling elevator to floor %d",
                   kPhase4ElevatorTargetFloor);
        }
        break;
      }
      case 3: {
        PublishPhase4Waypoint();
        if (!phase4_ride_requested_) {
          if (CallElevatorToFloor(kPhase4ElevatorTargetFloor, true, 30.0)) {
            phase4_ride_requested_ = true;
            phase4_step_ = 4;
            ROS_INFO("Phase4 step 3: elevator called to floor %d, waiting for arrival",
                     kPhase4ElevatorTargetFloor);
          } else {
            ROS_WARN_THROTTLE(2.0, "Phase4 step 3: waiting to call elevator to floor %d",
                              kPhase4ElevatorTargetFloor);
          }
        }
        break;
      ...
      case 4: {
        const double floor2_z_threshold = kPhase4_3WaypointZ - 0.5;
        if (robot_position_.z >= floor2_z_threshold) {
          phase4_step_ = 5;
          phase4_waypoint_.x = kPhase4_3WaypointX;
          phase4_waypoint_.y = kPhase4_3WaypointY;
          phase4_waypoint_.z = kPhase4_3WaypointZ;
          ROS_INFO("Phase4 step 4: elevator at floor 2 (z=%.2f), goto [%.2f, %.2f, %.2f]",
                   robot_position_.z, phase4_waypoint_.x, phase4_waypoint_.y,
                   phase4_waypoint_.z);
          PublishPhase4Waypoint();
        } else {
          ROS_WARN_THROTTLE(2.0,
                            "Phase4 step 4: waiting for elevator at floor 2 (robot z=%.2f, need >= %.2f)",
                            robot_position_.z, floor2_z_threshold);
        }
        break;
      }
      case 5: {
        PublishPhase4Waypoint();
        const double dist_floor2_exit =
            misc_utils_ns::PointXYDist(robot_position_, phase4_waypoint_);
  ...
  if (exploringPhase_ == 5) {
    ...
    PublishPhase5Waypoint();
    if (!phase5_arrived_) {
      const double dist_to_phase5_waypoint =
          misc_utils_ns::PointXYDist(robot_position_, phase5_waypoint_);
      if (dist_to_phase5_waypoint < kPhaseArrivalDist) {
        phase5_arrived_ = true;
        ROS_INFO("Phase5 complete: arrived at [%.2f, %.2f, %.2f]",
                 phase5_waypoint_.x, phase5_waypoint_.y, phase5_waypoint_.z);
        InitPhase6();
      }
    }
  ...
  if (exploringPhase_ == 7) {
    ...
    switch (phase7_step_) {
      case 0: {
        if (phase7_elevator_ready_) {
          phase7_step_ = 1;
          ROS_INFO("Phase7 step 1: approach outside waypoint [%.2f, %.2f, %.2f]",
                   phase7_waypoint_.x, phase7_waypoint_.y, phase7_waypoint_.z);
          PublishPhase7Waypoint();
          break;
        }
        if (TrySetupPhase7Elevator()) {
          phase7_step_ = 1;
          phase7_waypoint_.x = kPhase7_1WaypointX;
          phase7_waypoint_.y = kPhase7_1WaypointY;
          phase7_waypoint_.z = kPhase7_1WaypointZ;
          ROS_INFO("Phase7 step 1 (fallback setup): approach outside waypoint [%.2f, %.2f, %.2f]",
                   phase7_waypoint_.x, phase7_waypoint_.y, phase7_waypoint_.z);
          PublishPhase7Waypoint();
        } else {
          ROS_WARN_THROTTLE(2.0,
                            "Phase7 step 0 (fallback): waiting for elevator setup (all doors + floor 1)");
        }
        break;
      }
      case 1: {
        PublishPhase7Waypoint();
        const double dist_outside =
            misc_utils_ns::PointXYDist(robot_position_, phase7_waypoint_);
        if (dist_outside < kPhaseArrivalDist) {
          phase7_step_ = 2;
          phase7_waypoint_.x = kPhase7_2WaypointX;
          phase7_waypoint_.y = kPhase7_2WaypointY;
          phase7_waypoint_.z = kPhase7_2WaypointZ;
          ROS_INFO("Phase7 step 2: enter car waypoint [%.2f, %.2f, %.2f]",
                   phase7_waypoint_.x, phase7_waypoint_.y, phase7_waypoint_.z);
        }
        break;
      }
      case 2: {
        ...
        if (dist_inside < kPhaseArrivalDist) {
          phase7_step_ = 3;
          ROS_INFO("Phase7 step 3: calling elevator to floor %d",
                   kPhase7ElevatorTargetFloor);
        }
        break;
      }
      case 3: {
        PublishPhase7Waypoint();
        if (!phase7_ride_requested_) {
          if (CallElevatorToFloor(kPhase7ElevatorTargetFloor, true, 30.0)) {
            phase7_ride_requested_ = true;
            phase7_step_ = 4;
            ROS_INFO("Phase7 step 3: elevator called to floor %d, waiting for arrival",
                     kPhase7ElevatorTargetFloor);
          } else {
            ROS_WARN_THROTTLE(2.0, "Phase7 step 3: waiting to call elevator to floor %d",
                              kPhase7ElevatorTargetFloor);
          }
        }
        break;
      ...
      case 4: {
        const double floor3_z_threshold = kPhase7_3WaypointZ - 0.5;
        if (robot_position_.z >= floor3_z_threshold) {
          phase7_step_ = 5;
          phase7_waypoint_.x = kPhase7_3WaypointX;
          phase7_waypoint_.y = kPhase7_3WaypointY;
          phase7_waypoint_.z = kPhase7_3WaypointZ;
          ROS_INFO("Phase7 step 4: elevator at floor 3 (z=%.2f), goto [%.2f, %.2f, %.2f]",
                   robot_position_.z, phase7_waypoint_.x, phase7_waypoint_.y,
                   phase7_waypoint_.z);
          PublishPhase7Waypoint();
        } else {
          ROS_WARN_THROTTLE(2.0,
                            "Phase7 step 4: waiting for elevator at floor 3 (robot z=%.2f, need >= %.2f)",
                            robot_position_.z, floor3_z_threshold);
        }
        break;
      }
      case 5: {
        PublishPhase7Waypoint();
        const double dist_floor3_exit =
            misc_utils_ns::PointXYDist(robot_position_, phase7_waypoint_);
  ...
  if (exploringPhase_ == 8) {
    ...
    PublishPhase8Waypoint();
    if (!phase8_arrived_) {
      const double dist_to_phase8_waypoint =
          misc_utils_ns::PointXYDist(robot_position_, phase8_waypoint_);
      if (dist_to_phase8_waypoint < kPhaseArrivalDist) {
        phase8_arrived_ = true;
        ROS_INFO("Phase8 complete: arrived at [%.2f, %.2f, %.2f]",
                 phase8_waypoint_.x, phase8_waypoint_.y, phase8_waypoint_.z);
        InitPhase9();
      }
    }
...
void SensorCoveragePlanner3D::UpdateRoomLabel()
{
  ...
  for (auto &id_to_room_node : representation_->GetRoomNodesMapMutable())
  {
    ...
    else
    {
      ...
      hybrid_planner::RoomType room_type_msg;
...
void SensorCoveragePlanner3D::CheckObjectFound()
{
  ...
  for (auto &id_object_node_pair : representation_->GetObjectNodeRepMapMutable())
  {
    ...
    if (object_node.label_ == target_object_)
    {
      ...
      if (!kUseVlm)
      {
        auto found_object_msg = boost::make_shared<hybrid_planner::TargetObject>();
      ...
      if (spatial_condition_=="")
      {
        ...
        hybrid_planner::TargetObject target_object_msg;
        ...
        // // TODO: Unit Test
        // auto found_object_msg = std::make_shared<hybrid_planner::TargetObject>();
        // found_object_msg->header.stamp = ros::Time::now();
        // found_object_msg->object_id = object_node.object_id_[0];
        // found_object_msg->object_label = object_node.label_;
        // found_object_msg->img_path = object_node.img_path_;
      ...
      else
      {
        ...
        // publish a targrt object query
        hybrid_planner::TargetObjectWithSpatial target_object_msg;
        target_object_msg.header.stamp = ros::Time::now();
        target_object_msg.object_id = object_node.object_id_[0];
        target_object_msg.object_label = object_node.label_;
        target_object_msg.img_path = object_node.img_path_;
...
void SensorCoveragePlanner3D::CheckAnchorObjectFound()
{
  ...
  for (auto &id_object_node_pair : representation_->GetObjectNodeRepMapMutable())
  {
    ...
    if (object_node.label_ == anchor_object_)
    {
      ...
      // publish a targrt object query
      hybrid_planner::TargetObject anchor_object_msg;
