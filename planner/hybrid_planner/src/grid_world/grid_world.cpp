...
GridWorld::GridWorld(ros::NodeHandle* nh) : initialized_(false), use_keypose_graph_(false)
{
  ...
  door_position_ = geometry_msgs::Point();
...
{
  nh->param("kGridWorldXNum", kRowNum, kRowNum);
  nh->param("kGridWorldYNum", kColNum, kColNum);
  nh->param("kGridWorldZNum", kLevelNum, kLevelNum);
  int viewpoint_number = ([&](){ int _v=0; nh->param("viewpoint_manager/number_x", _v, _v); return _v; }());
  double viewpoint_resolution = ([&](){ double _v=0.0; nh->param("viewpoint_manager/resolution_x", _v, _v); return _v; }());
  kCellSize = viewpoint_number * viewpoint_resolution / 5;
  nh->param("kGridWorldCellHeight", kCellHeight, kCellHeight);
  nh->param("kGridWorldNearbyGridNum", KNearbyGridNum, KNearbyGridNum);
  nh->param("kMinAddPointNumSmall", kMinAddPointNumSmall, kMinAddPointNumSmall);
  nh->param("kMinAddPointNumBig", kMinAddPointNumBig, kMinAddPointNumBig);
  nh->param("kMinAddFrontierPointNum", kMinAddFrontierPointNum, kMinAddFrontierPointNum);
  nh->param("kCellExploringToCoveredThr", kCellExploringToCoveredThr, kCellExploringToCoveredThr);
  nh->param("kCellCoveredToExploringThr", kCellCoveredToExploringThr, kCellCoveredToExploringThr);
  nh->param("kCellExploringToAlmostCoveredThr", kCellExploringToAlmostCoveredThr, kCellExploringToAlmostCoveredThr);
  nh->param("kCellAlmostCoveredToExploringThr", kCellAlmostCoveredToExploringThr, kCellAlmostCoveredToExploringThr);
  nh->param("kCellUnknownToExploringThr", kCellUnknownToExploringThr, kCellUnknownToExploringThr);

  nh->param("room_resolution", room_resolution_, room_resolution_);
  nh->param("rolling_occupancy_grid/resolution_x", occupancy_grid_resolution_, occupancy_grid_resolution_);
  room_voxel_dimension_.x() = ([&](){ int _v=0; nh->param("room_x", _v, _v); return _v; }());
  room_voxel_dimension_.y() = ([&](){ int _v=0; nh->param("room_y", _v, _v); return _v; }());
  room_voxel_dimension_.z() = ([&](){ int _v=0; nh->param("room_z", _v, _v); return _v; }());
...
bool GridWorld::HasExploringCellsInRoom(int room_id) const
{
  std::vector<int> cell_indices;
  GetExploringCellIndicesInRoom(room_id, cell_indices);
  return !cell_indices.empty();
}

void GridWorld::GetExploringCellIndicesInRoom(int room_id, std::vector<int>& cell_indices) const
{
  cell_indices.clear();
  if (room_id <= 0 || room_mask_.empty())
  {
    return;
  }
  ...
  for (int i = 0; i < subspaces_->GetCellNumber(); i++)
  {
    ...
    if (connection_point_voxel.x() < 0 || connection_point_voxel.x() >= room_mask_.rows ||
        connection_point_voxel.y() < 0 || connection_point_voxel.y() >= room_mask_.cols)
    {
      continue;
    }
    if (room_mask_.at<int>(connection_point_voxel.x(), connection_point_voxel.y()) == room_id)
    {
...
void GridWorld::SetPlanningBoundary(double x_min, double x_max,
                                    double y_min, double y_max)
{
  planning_boundary_enabled_ = true;
  planning_x_min_ = x_min;
  planning_x_max_ = x_max;
  planning_y_min_ = y_min;
  planning_y_max_ = y_max;
  for (int i = 0; i < subspaces_->GetCellNumber(); ++i)
  {
    if (subspaces_->GetCell(i).GetStatus() == CellStatus::NOGO)
    {
      subspaces_->GetCell(i).SetStatus(CellStatus::UNSEEN);
    }
  }
}
...
void GridWorld::UpdateCellStatus(const std::shared_ptr<viewpoint_manager_ns::ViewPointManager>& viewpoint_manager)
{
  ...
  for (const auto& cell_ind : neighbor_cell_indices_)
  {
    ...
    if (planning_boundary_enabled_ &&
        (cell_position.x < planning_x_min_ ||
         cell_position.x > planning_x_max_ ||
         cell_position.y < planning_y_min_ ||
         cell_position.y > planning_y_max_))
    {
      subspaces_->GetCell(cell_ind).SetStatus(CellStatus::NOGO);
      continue;
    }
    ...
    int non_covered_viewpoint_count = 0;
    ...
    const bool has_non_covered_viewpoint = non_covered_viewpoint_count > 0;
    const bool freeze_exploring_to_covered = transit_across_room_;

    // Exploring to Covered
    if (!freeze_exploring_to_covered &&
    ...
    else if (!freeze_exploring_to_covered &&
             subspaces_->GetCell(cell_ind).GetStatus() == CellStatus::EXPLORING && selected_viewpoint_count == 0 &&
             candidate_count > 0 && in_room && !has_non_covered_viewpoint)
    {
      almost_covered_cell_indices_.push_back(cell_ind);
    }
             ...
             (selected_viewpoint_count > 0 ||
              (in_room && candidate_count > 0 && has_non_covered_viewpoint)))
    {
      subspaces_->GetCell(cell_ind).SetStatus(CellStatus::EXPLORING);
      almost_covered_cell_indices_.erase(
          ...
          almost_covered_cell_indices_.end());
             ...
             subspaces_->GetCell(cell_ind).GetStatus() == CellStatus::EXPLORING && candidate_count == 0 && in_room)
    {
      // First visit
    ...
    std::vector<int>& ordered_cell_indices, const std::shared_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph)
{
  ...
  std::vector<geometry_msgs::Point> exploring_cell_positions_global;
  std::vector<int> exploring_cell_indices_global;
  for (int i = 0; i < subspaces_->GetCellNumber(); i++)
  {
    const geometry_msgs::Point cell_position = GetCellPosition(i);
    if (planning_boundary_enabled_ &&
        (cell_position.x < planning_x_min_ ||
         cell_position.x > planning_x_max_ ||
         cell_position.y < planning_y_min_ ||
  ...
  {
    nav_msgs::Path across_room_path;
    if (transit_across_room_)
    {
    ...
    {
      // ROS_ERROR_STREAM(//                     "GridWorld::SolveGlobalTSP: Cannot find path across room, size: "
  ...
  {
    ROS_INFO("GridWorld::SolveGlobalTSP: Found anchor object, solving TSP");
  ...
  if (exploring_cell_indices.empty())
  {
    ...
    nav_msgs::Path room_finished_path;
    geometry_msgs::Point home_position = keypose_graph->GetFirstKeyposePosition();
