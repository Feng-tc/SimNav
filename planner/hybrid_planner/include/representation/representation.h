...
{
public:
  explicit ViewPointRep(int id, double x, double y, double z, pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr &cloud, pcl::PointCloud<pcl::PointXYZI>::Ptr &covered_cloud, const ros::Time &timestamp);
  explicit ViewPointRep(int id, const geometry_msgs::Point &position, pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr &cloud, pcl::PointCloud<pcl::PointXYZI>::Ptr &covered_cloud, const ros::Time &timestamp);
...
// private:
  ...
  int room_id_;
  ros::Time timestamp_; // Timestamp of the viewpoint
};
...
{
public:
  // explicit RoomNodeRep(const int id, const cv::Mat &room_mask, const geometry_msgs::PolygonStamped &polygon);
  // explicit RoomNodeRep(const int id, const std::vector<cv::Point> &points, const geometry_msgs::PolygonStamped &polygon);
  explicit RoomNodeRep(const int id, const std::vector<cv::Point> &points);
  // explicit RoomNodeRep(const hybrid_planner::RoomNode msg);
  RoomNodeRep() = default;
  ~RoomNodeRep() = default;

  // void UpdateRoomNode(const cv::Mat &room_mask, const geometry_msgs::PolygonStamped &polygon);
  // void UpdateRoomNode(const cv::Mat &room_mask);
  void UpdateRoomNode(const std::vector<cv::Point> &points, const geometry_msgs::PolygonStamped &polygon);
  void UpdateRoomNode(const std::vector<cv::Point> &points);
  void UpdatePolygon(const geometry_msgs::PolygonStamped &polygon);
  void UpdateCentroid(Eigen::Vector3f &centroid);
  void RemovePoint(const cv::Point &point);
  bool InRoom(const geometry_msgs::Point &point);
  // void HeritageAttributes(const RoomNodeRep &prev_room_node);
  void UpdateRoomNode(const hybrid_planner::RoomNode msg);
  ...
  void SetAnchorPoint(const geometry_msgs::Point &anchor_point)
  {
    anchor_point_ = anchor_point;
  }
  const geometry_msgs::Point & GetAnchorPoint() const
  {
  ...
  const geometry_msgs::PolygonStamped& GetPolygon() const { return polygon_; }
  ...
  geometry_msgs::Point anchor_point_;
...
{
public:
  explicit ObjectNodeRep(const hybrid_planner::ObjectNode::ConstPtr msg);
  ...
  const geometry_msgs::Point& GetPosition() const { return position_; }
  const sensor_msgs::PointCloud2& GetCloud() const { return cloud_; }
  ...
  void UpdateObjectNode(const hybrid_planner::ObjectNode::ConstPtr msg);
  ...
  geometry_msgs::Point position_;
  boost::array<geometry_msgs::Point, 8> bbox3d_; 
  ...
  ros::Time timestamp_; // Timestamp of the object node
...
{
public:
  explicit Representation(ros::NodeHandle* nh, std::string world_frame_id = "map");
   ...
   */
  int AddViewPointRep(const geometry_msgs::Point &position, 
   ...
   * @throws std::out_of_range if index is invalid
   */
  geometry_msgs::Point GetViewPointRepNodePos(int index) const;
   ...
   */
  RoomNodeRep& AddRoomNode(const hybrid_planner::RoomNode& msg);
...
private:
  ...
  // ==================== Private Methods ====================
  void AddViewPointRepNode(const geometry_msgs::Point &position, 
                          pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr &cloud, 
                          pcl::PointCloud<pcl::PointXYZI>::Ptr &covered_cloud, 
                          const ros::Time &timestamp);
};
