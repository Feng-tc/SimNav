...
#include "grid/grid.h"
#include <hybrid_planner/ObjectNode.h>
#include <hybrid_planner/ObjectNodeList.h>
#include <hybrid_planner/ViewpointRep.h>
#include <visualization_msgs/MarkerArray.h>
#include <visualization_msgs/Marker.h>
#include <xmlrpcpp/XmlRpcValue.h>
#include <opencv2/opencv.hpp>
#include <cv_bridge/cv_bridge.h>
#include <hybrid_planner/RoomNode.h>
#include <hybrid_planner/RoomNodeList.h>
#include <hybrid_planner/RoomType.h>
#include <hybrid_planner/TargetObjectInstruction.h>
#include <hybrid_planner/TargetObject.h>
#include <hybrid_planner/TargetObjectWithSpatial.h>
#include <filesystem>
...
struct ConfiguredRoomTask {
  int floor = 0;
  std::vector<geometry_msgs::Point> entry_waypoints;
  std::vector<geometry_msgs::Point> exit_waypoints;
  std::array<double, 4> bbox{{0.0, 0.0, 0.0, 0.0}};
  int explored_threshold = 90;
  double timeout = 120.0;
};

struct ConfiguredElevatorTrip {
...
private:
  ...
  std::string kElevatorId;
  int kElevatorServedFloorCount;
  ...
  double kPhase7_1WaypointX;
  double kPhase7_1WaypointY;
  double kPhase7_1WaypointZ;
  double kPhase7_2WaypointX;
  double kPhase7_2WaypointY;
  double kPhase7_2WaypointZ;
  double kPhase7_3WaypointX;
  double kPhase7_3WaypointY;
  double kPhase7_3WaypointZ;
  double kPhase8WaypointX;
  double kPhase8WaypointY;
  double kPhase8WaypointZ;
  ...
  int kPhase7NoRoomCycles;
  int kPhase7ElevatorTargetFloor;
  ...
  geometry_msgs::Point phase7_waypoint_;
  geometry_msgs::Point phase8_waypoint_;
  ...
  // Configured-room mission state.
  MissionMode mission_mode_;
  std::vector<ConfiguredRoomTask> room_tasks_;
  ConfiguredElevatorTrip elevator_trips_[2];
  int current_room_task_index_;
  int current_floor_;
  bool room_exiting_;
  int exit_room_task_index_;
  size_t entry_waypoint_index_;
  size_t exit_waypoint_index_;
  bool room_no_exploring_cells_;
  bool room_has_valid_coverage_update_;
  ros::Time room_exploration_start_time_;
  int elevator_trip_index_;
  int elevator_step_;
  bool elevator_ride_requested_;
  ...
  ros::Publisher reset_room_seg_pub_;
  // Debug
  ros::Publisher
  ...
  void PublishPhase7Waypoint();
  void PublishPhase8Waypoint();
  ...
  void RunCoveragePlanningCycle();
  void HandlePhase1Door();
  void HandleWaypointNavigation();
  void HandleElevatorTransit();
  void TransitionToWaypointNavigation();
  void TransitionToRoomExploration();
  void TransitionToElevator(int target_floor);
  void CompleteCurrentRoom(const std::string& reason);
  void FinishMission();
  void PublishMissionWaypoint(const geometry_msgs::Point& waypoint);
  bool ArrivedAtMissionWaypoint(const geometry_msgs::Point& waypoint) const;
  void ApplyCurrentRoomBoundary();
  bool UsesConfiguredRoomMission() const;
  bool ClipWaypointToRoomInterior(geometry_msgs::Point* waypoint,
                                  const std::array<double, 4>& bbox) const;
  static geometry_msgs::Polygon MakeRoomBoundaryPolygon(
      const std::array<double, 4>& bbox, double z, double inset);
  void PublishConfiguredMissionMarkers();
  void ResetConfiguredRoomContext();
  ...
  static bool LoadPointParam(const ros::NodeHandle& nh,
  ...
  ros::Publisher configured_mission_marker_pub_;
  ...
  // Callback functions
  void ObjectNodeListCallback(const hybrid_planner::ObjectNodeList::ConstPtr& msg);
  void DoorCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& door_cloud_msg);
  void RoomNodeListCallback(const hybrid_planner::RoomNodeList::ConstPtr& room_node_list_msg);
  ...
  void RoomTypeCallback(const hybrid_planner::RoomType::ConstPtr& msg);
  void KeyboardInputCallback(const std_msgs::String::ConstPtr& msg);
  void TargetObjectInstructionCallback(const hybrid_planner::TargetObjectInstruction::ConstPtr& msg);
  void TargetObjectCallback(const hybrid_planner::TargetObject::ConstPtr& msg);
  void AnchorObjectCallback(const hybrid_planner::TargetObject::ConstPtr& msg);
  ...
  hybrid_planner::ViewpointRep viewpoint_rep_msg_;
