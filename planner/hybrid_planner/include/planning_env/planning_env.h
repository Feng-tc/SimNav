...
{
public:
  PlanningEnv(ros::NodeHandle* nh, std::string world_frame_id = "map");
  ~PlanningEnv() = default;
  ...
  template <class PCLPointType>
  void UpdateKeyposeCloudForCollisionCheck(
      typename pcl::PointCloud<PCLPointType>::Ptr& keypose_cloud) {
    if (keypose_cloud->points.empty()) {
      ROS_WARN(
          "PlanningEnv::UpdateKeyposeCloudForCollisionCheck(): keypose cloud "
          "empty");
      return;
    }

    pcl::copyPointCloud<PCLPointType, PlannerCloudPointType>(
        *keypose_cloud, *(keypose_cloud_->cloud_));
    ...
    keypose_cloud_stack_[keypose_cloud_count_]->clear();
    *keypose_cloud_stack_[keypose_cloud_count_] = *keypose_cloud_->cloud_;
    keypose_cloud_count_ =
        (keypose_cloud_count_ + 1) % parameters_.kKeyposeCloudStackNum;
    stacked_cloud_->cloud_->clear();
    for (int i = 0; i < parameters_.kKeyposeCloudStackNum; i++) {
      *(stacked_cloud_->cloud_) += *keypose_cloud_stack_[i];
    }
    ...
    *vertical_surface_cloud_stack_[keypose_cloud_count_] =
    ...
    for (int i = 0; i < parameters_.kKeyposeCloudStackNum; i++) {
      *(stacked_vertical_surface_cloud_->cloud_) +=
          *vertical_surface_cloud_stack_[i];
    }
    stacked_cloud_downsizer_.Downsize(
        stacked_vertical_surface_cloud_->cloud_,
        parameters_.kSurfaceCloudDwzLeafSize,
