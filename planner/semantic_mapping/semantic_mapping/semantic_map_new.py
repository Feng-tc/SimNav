...
from .tools import ros_utils as ros_utils
...
def serialize_objs_to_bag(writer, obj_mapper, stamp: float, raw_cloud=None, odom=None):
    """Legacy bag export helper; requires ROS2 rosbag2 and is unused at runtime."""
    raise NotImplementedError('serialize_objs_to_bag is not supported in ROS1 builds')
...
class ObjMapper():
    ...
    def flush_deleted_objects_batch(self, stamp):
        ...
        deleted_list_msg.header.stamp = rospy.Time(seconds, nanoseconds)
    ...
    def to_ros2_msgs(self, stamp, viewpoint_id):
        ...
        for single_obj in self.single_obj_list:
            ...
            if label == self.target_object and self.use_vlm and not single_obj.is_asked_vlm:
            ...
            obj_marker = ros_utils.create_wireframe_marker(
                center=aabb.get_center(),
                extent=aabb.get_extent(),
                yaw=0.0,
                ns='bbox',
            ...
            text_msg = ros_utils.create_text_marker(
                center=center,
            ...
            object_node_msg.header.stamp = rospy.Time(seconds, nanoseconds)
            ...
            ros_pcd = ros_utils.create_point_cloud(
            ...
            object_node_msg.is_asked_vlm = single_obj.is_asked_vlm or (
                not self.use_vlm and label == self.target_object
            )
        ...
        if self.object_node_pub is not None and len(object_nodes_list) > 0:
            ...
            object_node_list_msg.header.stamp = rospy.Time(seconds, nanoseconds)
        ...
        if len(points_list) != 0:
            ...
            ros_pcd = ros_utils.create_colored_point_cloud(
                points=points,
                colors=colors,
                seconds=seconds,
                nanoseconds=nanoseconds,
    ...
    def check_target_objects(self):
        ...
        for single_obj in self.single_obj_list:
            ...
            label = single_obj.get_dominant_label()
            if (label == self.target_object or label == self.anchor_object) and self.use_vlm and not single_obj.is_asked_vlm and single_obj.best_image_score > 500:
                single_target_obj = {
