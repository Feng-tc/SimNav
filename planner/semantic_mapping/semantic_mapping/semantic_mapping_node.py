...
def load_models():
    ...
    checkpoint = str(pkg_dir / "external/sam2/checkpoints/sam2.1_hiera_base_plus.pt")
    model_cfg = str(pkg_dir / "external/sam2/sam2/configs/sam2.1/sam2.1_hiera_b+.yaml")
...
class MappingNode:
    def __init__(self, mask_predictor):
        ...
        self.target_object = rospy.get_param('~target_object', 'refrigerator')
        self.platform = rospy.get_param('~platform', 'mecanum_sim')
        self.detection_linear_state_time_bias = rospy.get_param('~detection_linear_state_time_bias', 0.0)
        self.detection_angular_state_time_bias = rospy.get_param('~detection_angular_state_time_bias', 0.0)
        self.ANNOTATE = rospy.get_param('~annotate_image', True)
        self.grounding_score_thresh = rospy.get_param('~grounding_score_thresh', 0.3)
        self.object_file_path = rospy.get_param('~object_file', _default_object_file())
        self.save_png = rospy.get_param('~save_png', False)
        ...
        self.cloud_sub = rospy.Subscriber('/registered_scan', PointCloud2, self.cloud_callback, queue_size=10)

        if self.platform == 'mecanum_bagfile':
            self.odom_sub = rospy.Subscriber(
                '/aft_mapped_to_init_incremental', Odometry, self.odom_callback, queue_size=50)
        else:
            self.odom_sub = rospy.Subscriber(
                '/state_estimation', Odometry, self.odom_callback, queue_size=50)

        self.viewpoint_sub = rospy.Subscriber(
            '/viewpoint_rep_header', ViewpointRep, self.viewpoint_timestamp_callback, queue_size=5)
        self.detection_result_sub = rospy.Subscriber(
            '/detection_result', DetectionResult, self.detection_result_callback, queue_size=50)
        self.object_type_answer_sub = rospy.Subscriber(
            '/object_type_answer', ObjectType, self.object_type_answer_callback, queue_size=50)
        self.target_object_instruction_sub = rospy.Subscriber(
            '/target_object_instruction', TargetObjectInstruction,
            self.target_object_instruction_callback, queue_size=10)
        self.keyboard_input_sub = rospy.Subscriber(
            '/keyboard_input', String, self.keyboard_input_callback, queue_size=10)

        self.object_type_query_pub = rospy.Publisher('/object_type_query', ObjectType, queue_size=50)

        self.detection_counter = 0
        self.mapping_timer = rospy.Timer(rospy.Duration(0.5), self.mapping_callback)

        self.obj_cloud_pub = rospy.Publisher('/obj_points', PointCloud2, queue_size=10)
        self.obj_box_pub = rospy.Publisher('/obj_boxes', MarkerArray, queue_size=10)
        self.obj_text_pub = rospy.Publisher('/obj_labels', MarkerArray, queue_size=10)
        self.annotated_image_pub = rospy.Publisher('/annotated_image', Image, queue_size=10)
        self.cloud_image_pub = rospy.Publisher('/cloud_image', Image, queue_size=10)
        self.object_node_pub = rospy.Publisher('/object_nodes_list', ObjectNodeList, queue_size=200)
    ...
    def cloud_callback(self, msg):
        with self.cloud_cbk_lock:
            ...
            stamp_seconds = msg.header.stamp.to_sec()
    ...
    def viewpoint_timestamp_callback(self, msg):
        ...
        with self.viewpoint_queue_lock:
            self.log_info(
                f'Received viewpoint id: {msg.viewpoint_id}, timestamp: {timestamp:.3f}')
            if timestamp not in self.viewpoint_queue and timestamp not in self.processed_viewpoints:
                ...
                self.log_info(
                    f'Added viewpoint timestamp to queue: {msg.viewpoint_id}, queue size: {len(self.viewpoint_queue)}')
            else:
                self.log_info(f'Skipping duplicate viewpoint timestamp: {msg.viewpoint_id}')
        ...
        with self.mapping_processing_lock:
            ...
            if self.ANNOTATE:
                ...
                if viewpoint_stamp_to_process is not None:
                    ...
                    self.log_info(f"Saved viewpoint images for timestamp: {timestamp_for_filename}")
                # else:
                ...
                ros_image.header.stamp = rospy.Time.from_sec(detection_stamp)
            ...
            if self.use_vlm and len(target_objs) > 0:
                self.log_info(f"Target objects {self.target_object} found: {target_objs}")
    ...
    def mapping_callback(self, _event=None):
            ...
            with self.viewpoint_queue_lock:
                if self.viewpoint_queue:
                    ...
                    if candidate_viewpoint_stamp > self.detection_results_stamps[0] and candidate_viewpoint_stamp < self.detection_results_stamps[-1]:
                        ...
                        self.log_info(
                            f'Processing viewpoint from queue: {viewpoint_id}, remaining in queue: {len(self.viewpoint_queue)}')
            ...
            if viewpoint_stamp_to_process is not None:
                ...
                for i, stamp in enumerate(self.detection_results_stamps):
                    ...
                    if diff < min_diff:
                        ...
                        closest_stamp = stamp
                self.log_info(
                    f'Viewpoint timestamp: {viewpoint_stamp_to_process:.3f}, '
                    f'closest detection: {closest_stamp:.3f}, '
                    f'time difference: {min_diff:.3f}s')
                if min_diff > 1.0:
                    self.log_info('Closest detection too far from viewpoint timestamp. Skipping this viewpoint.')
